                            ...
                            apr_off_t readbytes)
{
    ...
    if (!ctx) {
         ...
         * responses be useful?
         */
        if (!f->r->proxyreq && !f->r->offlinereq) {
        ...
        if (ctx->state == BODY_NONE && f->r->proxyreq != PROXYREQ_RESPONSE && f->r->offlinereq != OFFLINEREQ_RESPONSE) {
            e = apr_bucket_eos_create(f->c->bucket_alloc);
                                                           ...
                                                           apr_bucket_brigade *b)
{
    ...
    /* 计算状态行和响应头的大小:在同一个存储段组中 */
    apr_brigade_length(b2, 1, &length);
    r->response_bytes += length;
