                                            ...
                                            request_rec *r)
{
    ...
    apr_sockaddr_t *local_addr = c->remote_addr;
    ...
    while (backend_addr && !connected) {
        ...
        /* tproxy:透明代理需要使用客户端的源地址连接后端真实服务器 */
        if (conf->transp && local_addr) {
            rv = apr_socket_opt_set(newsock, APR_IP_TRANSPARENT, 1);
            if (rv != APR_SUCCESS && rv != APR_ENOTIMPL) {
                 ap_log_error(APLOG_MARK, APLOG_ERR, rv, s,
            ...
            if ((rv = apr_socket_bind(newsock, local_addr)) != APR_SUCCESS) {
                ap_log_error(APLOG_MARK, APLOG_ERR, rv, s,
                             "proxy: %s: attempt to bind to address %pI (%s) failed",
                             proxy_function,
                             local_addr,
                             worker->hostname);
            }
        ...
        if (rv != APR_SUCCESS) {
            ...
            loglevel = backend_addr->next ? APLOG_DEBUG : APLOG_ERR;
            ap_log_error(APLOG_MARK, loglevel, rv, s,
                         "proxy: %s: %pI attempt to connect to %pI (%s) failed",
                         proxy_function,
                         local_addr,
                         backend_addr,
                         worker->hostname);
