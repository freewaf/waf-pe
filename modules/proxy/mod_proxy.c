...
static int proxy_trans(request_rec *r)
{
    ...
    /* 透明代理需要替换后端真实服务器的地址为客户端连接的目标地址 */
    if (conf->transp) {
        fake = apr_psprintf(r->pool, "/");
        real = apr_psprintf(r->pool, "http://%s:%d/", r->connection->local_ip,
...
static void * merge_proxy_config(apr_pool_t *p, void *basev, void *overridesv)
{
    ...
    ps->transp = (overrides->transp_set == 0) ? base->transp : overrides->transp;
    ps->transp_set = overrides->transp_set || base->transp_set;
...
static const char *
set_proxy_transp(cmd_parms *parms, void *dummy, int flag)
{
    proxy_server_conf *psf =
    ap_get_module_config(parms->server->module_config, &proxy_module);
...
static const command_rec proxy_cmds[] =
{
    ...
    AP_INIT_FLAG("ProxyTransparent", set_proxy_transp, NULL, RSRC_CONF,
                 "on if the true to enable transparent proxy"),
