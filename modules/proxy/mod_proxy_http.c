                                   ...
                                   char *url, char *server_portstr)
{
     ...
     */
    if (!conf->transp) {
        if (PROXYREQ_REVERSE == r->proxyreq && conf->transp == FALSE) {
            const char *buf;
...
skip_body:
    /* tproxy */
    if (conf->transp) {
        /* Put request host here according to orginal request header */
        if (conf->preserve_host == 0) {
            if (ap_strchr_c(uri->hostname, ':')) { /* if literal IPv6 address */
                if (uri->port_str && uri->port != DEFAULT_HTTP_PORT) {
                    buf = apr_pstrcat(p, "Host: [", uri->hostname, "]:", 
                              ...
                              apr_port_t proxyport)
{
    ...
    if (ap_proxy_connect_backend(proxy_function, backend, worker, r)) {
