...
/* 命令默认值 */
#define DEF_DATADIR_VAL             "data"
#define DEF_DEBUGLOG_VAL            "logs/debug_log"
#define DEF_GEOLOOKUPDB_VAL         "/usr/apache-mips/GeoLiteCity.dat"
#define DEF_GSBLOOKUPDB_VAL         "/usr/apache-mips/GeoLiteCity.dat"
#define DEF_UNICODEMAPFILE_VAL      "data/unicode.mapping"
#define DEF_TMPDIR_VAL              "/tmp"
#define DEF_UPLOADDIR_VAL           "/tmp"
#define DEF_DRIVER_NAME             "sqlite3"
#define DEF_REQBODYACCESS_VAL       1
#define DEF_RESBODYACCESS_VAL       1
#define DEF_UPLOADFILEMODE_VAL      0640
#define DEF_UPLOADKEEPFILE_VAL      0
...
#define REQUEST_BODY_HARD_LIMIT                 1073741824L      /* 1GB   */
#define REQUEST_BODY_DEFAULT_INMEMORY_LIMIT     131072           /* 128KB */
#define REQUEST_BODY_DEFAULT_LIMIT              1073741824L      /* 1GB   */ /* 原先128M */
#define REQUEST_BODY_NO_FILES_DEFAULT_LIMIT     1048576          /* 1MB   */
#define RESPONSE_BODY_DEFAULT_INMEMORY_LIMIT    3145728          /* 3MB   */
#define RESPONSE_BODY_DEFAULT_LIMIT             1073741824L      /* 1GB   */
#define RESPONSE_BODY_HARD_LIMIT                1073741824L      /* 1GB   */
...
extern module AP_MODULE_DECLARE_DATA security_module;
...
extern DSOLOCAL unsigned long int dbm_timeout;

extern DSOLOCAL int *unicode_map_table;

#ifdef DEBUG_FILELOG
extern DSOLOCAL apr_file_t *auditlog_fd;
#endif
...
struct modsec_rec {
    ...
    unsigned int         white_list;
    unsigned int         black_list_flag;
    unsigned int         black_list_log;
    unsigned int         black_list_hitcount;
...
/* 裁减 */
    ...
    apr_hash_t          *var_fetch_cache;
    apr_pool_t          *var_fetch_cache_mptmp;
#endif
...
/* cookie字符串分隔符 */
enum msre_cookie_separatemark_s {
    MSRE_COOKIE_SEP_SEMICOLON,      /* 分隔符为';' */
    MSRE_COOKIE_SEP_COMMA,          /* 分隔符为',' */
    MSRE_COOKIE_SEP_END             /* 末尾无分隔符 */
};

/* cookie各个属性字段对应的value属性 */
typedef struct msre_cookie_attr_s {
    char hasequalmark;              /* 等号 */
    char hasleftquotmark;           /* 左双引号 */
    char hasrightquotmark;          /* 右双引号 */
    char separatemark;              /* 分隔符 */
    char *attrval;                  /* cookie属性字符串 */
} msre_cookie_attr_t;
...
apr_status_t DSOLOCAL modsecurity_request_body_store(modsec_rec *msr, const char *data, 
                        apr_size_t length, char **error_msg);

apr_status_t DSOLOCAL modsecurity_request_body_end(modsec_rec *msr, char **error_msg);

apr_status_t DSOLOCAL modsecurity_request_body_to_stream(modsec_rec *msr, const char *buffer, 
                        int buflen, char **error_msg);
...
const char DSOLOCAL *msc_alert_message(modsec_rec *msr, msre_actionset *actionset, 
                        const char *action_message, const char *rule_message);

void DSOLOCAL msc_record_attacklog(modsec_rec *msr, msre_var *var, msre_actionset *actionset);

void DSOLOCAL msc_alert(modsec_rec *msr, int level, msre_actionset *actionset, 
                const char *action_message, const char *rule_message);
