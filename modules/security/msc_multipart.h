...
struct multipart_data {
    ...
    /* mime boundary used to detect when parts end and begin */
    char                    *boundary;
    int                      boundary_count;

    /* internal buffer and other variables used while parsing */
    ...
    /* pointer that keeps track of a part while it is being built */
    multipart_part          *mpp;

    /* part parsing state; 0 means we are reading headers, 1 means we are collecting data */
    int                      mpp_state;
...
int DSOLOCAL multipart_process_chunk(modsec_rec *msr, const char *buf, unsigned int size, char **error_msg);
