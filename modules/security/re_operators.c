 ...
 *
 */
void msre_engine_op_register(msre_engine *engine, const char *name, fn_op_param_init_t fn1, 
        fn_op_execute_t fn2)
{
    msre_op_metadata *metadata = (msre_op_metadata *)apr_pcalloc(engine->mp, sizeof(msre_op_metadata));
    if (metadata == NULL) {
        return;
    }
...
/* -- Operators -- */
/* unconditionalMatch */
static int msre_op_unconditionalmatch_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, 
            char **error_msg)
{
...
/* noMatch */
static int msre_op_nomatch_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    *error_msg = "No match.";
...
static int msre_op_ipmatch_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    ...
    while (current) {
        if (apr_ipsubnet_test(current->ipsubnet, sa)) {
            *error_msg = apr_psprintf(msr->mp, "IPmatch \"%s\" matched \"%s\" at %s.", var->value, 
                            current->address, var->name);
            return 1;
        }
...
static int idnum_18_check(char *idnumber, int len)
{
    static const int wtable[17] = {7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2}; 
    int sum;
    int i;
    char checkchar;  
    ...
    /* 十七位数字本体码加权码求和公式S = Sum(Ai * Wi), i = 0, ... ,16 */
    for (i = 0, sum = 0; i < 17; i++) {
        sum += (idnumber[i] - '0') * wtable[i]; 
    }
    ...
    return (checkchar == idnumber[17]) ? 1 : 0;
}

/* 身份证号码校验 */
static int idnum_verify(apr_pool_t *pool, const char *idstr, int len) 
{
    const char *id_start;
    char *idnumber;
    int id_len;
    char iddd[3] = {0};   /* 天数格式dd */
    char idmm[3] = {0};   /* 月份格式mm */
    char idyyyy[5] = {0}; /* 年份格式yyyy */
    int year, day;
    int match = 1;
    ...
    if (id_len == 15 || id_len == 18) {
        idnumber = apr_pstrndup(pool, id_start, id_len);
        if (idnumber == NULL) {
            return -1;
        }
    } else {
    ...
    if (id_len == 15) {
        /* 15位身份证格式ddddddyymmddxxs */
        apr_snprintf(idmm, 3, "%s", idnumber +8);
        if (atoi(idmm) == 2) {
            apr_snprintf(idyyyy, 5, "19%s", idnumber + 6);
            year = atoi(idyyyy);
            if (!((year % 4 == 0) && (year % 100 != 0)) && !(year % 400 == 0)) {
                apr_snprintf(iddd, 3, "%s", idnumber + 10);
                day = atoi(iddd);
                if (day >= 29) {
                    match = 0;
                }
        ...
        /* 18位身份证格式ddddddyyyymmddxxsp */
        apr_snprintf(idmm, 3, "%s", idnumber + 10);
        if (atoi(idmm) == 2) {
            apr_snprintf(idyyyy, 5, "%s", idnumber + 6);
            year = atoi(idyyyy);
            if (!((year % 4 == 0) && (year % 100 != 0)) && !(year % 400 == 0)) {
                apr_snprintf(iddd, 3, "%s", idnumber + 12);
                day = atoi(iddd);
                if (day >= 29) {
                    match = 0;
                }
        ...
        if (match == 1) {
            if (idnum_18_check(idnumber, id_len) == 0) {
                match = 0;
            }
        }
    } else {
...
/* 去除身份证号或者信用卡号两端的非数字字符 */
static void escape_notdigital(int card_flag, char *offset, int *start, int *end)
{
    char *match_start, *match_end;

    match_start = offset + *start;
    match_end = offset + *end;

    /* 去掉头部非数字字符 */
    while (match_start < match_end && !apr_isdigit(*match_start)) {
        match_start++;
        (*start)++;
    }
    ...
    if (card_flag == 'n') { 
        /* 身份证号码 */
        while (match_end > match_start) {
            if (apr_isdigit(*match_end)) {
                break;
            }
            if ((*match_end == 'x' || *match_end == 'X') 
                && (match_end - match_start + 1) == 18) {
                break;
            }
            match_end--;
            (*end)--;
        ...
        /* 其它类型号码 */
        while (match_end > match_start && !apr_isdigit(*match_start)) {
            match_end--;
            (*end)--;
        }
    }
...
static int msre_op_rsub_param_init(msre_rule *rule, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    if (strcmp(rule->op_name, "rsubSen") == 0) {
        is_sen_rsub = 1;
    }
    ...
    if (flags) {
        while (*flags) {
            ...
            case 'c':
                if (is_sen_rsub) {
                    if (++cardnum > 1) {
                        *error_msg = apr_psprintf(rule->ruleset->mp, "Regex flag support just one card flag");
                        return -1;
                    }
                    rule->card_check_flag = 'c';
                } else {
        ...
        if (is_sen_rsub && cardnum == 0) {
            *error_msg = apr_psprintf(rule->ruleset->mp, "Regex flag need one card flag");
            return -1;
        }
    }
...
#if AP_SERVER_MAJORVERSION_NUMBER > 1 && AP_SERVER_MINORVERSION_NUMBER > 0
        regex = ap_pregcomp(rule->ruleset->mp, pattern, AP_REG_EXTENDED | (ignore_case ? AP_REG_ICASE : 0));
#else
        regex = ap_pregcomp(rule->ruleset->mp, pattern, REG_EXTENDED | (ignore_case ? REG_ICASE : 0));
#endif
...
static int msre_op_rsub_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) 
{
    ...
    int output_body = 0, response_header = 0, response_status = 0, input_body = 0, sl;
    char *res_parm = NULL;
    char *p;
    int changed = 0;
#if AP_SERVER_MAJORVERSION_NUMBER > 1 && AP_SERVER_MINORVERSION_NUMBER > 0
    ...
    *error_msg = NULL;
    ...
    if(strcmp(var->name,"STREAM_OUTPUT_BODY") == 0 )  {
        output_body = 1;
    } else if(strstr(var->name,"RESPONSE_HEADERS") != NULL) {
        response_header = 1;
        p = strchr(var->name, ':');
        if (p == NULL) {
            *error_msg = "Operator rsub dosen't work with this variables";
            return -1;
        }
        res_parm = apr_pstrdup(msr->r->pool, p + 1);            
    } else if(strcmp(var->name,"RESPONSE_STATUS") == 0) {
        response_status = 1; 

    } else if(strcmp(var->name,"STREAM_INPUT_BODY") == 0 )  {
        input_body = 1;
    } else  {
        msr_log(msr, 9," Operator rsub only works with STREAM_*/RESPONSE_HEADERS/RESPONSE_STATUS variables");
        *error_msg = "Operator rsub error";
        return -1;
    }
...
nextround:
    ...
    for (offset = (char*)var->value; !ap_regexec(rule->sub_regex, offset, AP_MAX_REG_MATCH, pmatch, 0); ) {
        ...
        /* fixup: 当p_len为0的时候，offset将无法往下偏移，造成死循环，所以在此处加判断直接退出 */
        if (p_len == 0) {
            break;
        }
    ...
    *data_out=0;
    ...
    /* 如果没有匹配上任何一个字段，则不进行替换直接返回 */
    if (changed == 0) {
        msr_log(msr, 9, "rsub: Nothing to change");
        return 0;
    }
    ...
    if (response_status == 1) {
        int status;                      
        status = atoi(data);
        if (msr->r->status != status) {
            msr->r->status = status; 
            msr->r->status_line = ap_get_status_line(status);
        }
    }

    if (msr->r->headers_out != NULL && response_header == 1) {
        char *res_header_data;
        res_header_data = apr_pstrndup(msr->r->pool, data, size);
        apr_table_setn(msr->r->headers_out, res_parm, res_header_data);
        if (msr->r->err_headers_out != NULL) {
            if (apr_table_get(msr->r->err_headers_out, res_parm) != NULL) {
                apr_table_setn(msr->r->err_headers_out, res_parm, res_header_data);   
            }
        }
    }
...
/**
* \brief Execution function to rsubSen operator
*
* \param msr Pointer internal modsec request structure
* \param rule Pointer to the rule
* \param var Pointer to variable structure
...
* \retval 0 On No Match
*/
static int msre_op_rsubSen_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) 
{
    msc_string *str = (msc_string *)apr_pcalloc(msr->mp, sizeof(msc_string));
    msc_string *re_pattern = (msc_string *)apr_pcalloc(msr->mp, sizeof(msc_string));
    char *offset = NULL;
    ...
    unsigned int size = 0;
    unsigned int maxsize=0;
    int output_body = 0, response_header = 0, sl;
    int is_effect_num = 0;
    char *res_parm = NULL;
    char *p;
    int changed = 0;
    ...
    *error_msg = NULL;
    ...
    if(strcmp(var->name,"STREAM_OUTPUT_BODY") == 0 )  {
        ...
        p = strchr(var->name, ':');
        if (p == NULL) {
            *error_msg = "Operator rsubSen dosen't work with this variables";
            return -1;
        }
        res_parm = apr_pstrdup(msr->r->pool, p + 1);            
    } else {
        msr_log(msr,9,"Operator rsubSen only works with STREAM_OUTPUT_BODY variables");
        *error_msg = "Operator rsubSen error";
        return -1;
    }
...
nextround:
    ...
    for (offset = (char*)var->value; !ap_regexec(rule->sub_regex, offset, AP_MAX_REG_MATCH, pmatch, 0); ) {
        //Copy of data before the regex match
        /* 
         * pmatch [0].rm_so为匹配上的第一个字符相对于offset的偏移
         * aaabbbXXXXXXXXccc
         *       |       |
         *       so=6    eo=14
         */
        int i;
        int s = pmatch [0].rm_so;
        int e = pmatch [0].rm_eo - 1;
        
        /* 去掉头尾匹配上的非数字字符 */
        escape_notdigital(rule->card_check_flag, offset, &s, &e);
        if (size + s > maxsize) {
            maxsize*=2;
            goto nextround;
        }
        ...
        switch (rule->card_check_flag) {
        case 'c':
            is_effect_num = luhn_verify(offset + s, p_len);
            break;
        case 'n':
            is_effect_num = idnum_verify(msr->mp, offset + s, p_len);
            if (is_effect_num == -1) {
                *error_msg = "Internal Error: cannot allocate memory";
                return -1;
            }
            break;
        default:
            *error_msg = "Operator rsubSen can not support this card flag";
            return -1;
        }        
        ...
        if (is_effect_num) {
            ...
            memcpy(data_out, offset + s, p_len);
            data_out += p_len;
            size += p_len;
    ...
    *data_out = 0;
    ...
    /* 对于无变化的响应体不进行替换直接返回 */
    if (changed == 0) {
        msr_log(msr, 9, "rsubSen: Nothing to change");
        return 0;
    }
    ...
    if(msr->stream_output_data != NULL && output_body == 1) {
        ...
        if(msr->stream_output_data == NULL) {
            *error_msg = "Operator rsubSen malloc error";
            return -1;
        }
    ...
    if (! *error_msg) {
        *error_msg = apr_psprintf(msr->mp, "Operator rsubSen succeeded.");
    }
...
static int msre_op_rx_param_init(msre_rule *rule, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    /* Compile pattern */
    if(strstr(pattern,"%{") == NULL)    {
        regex = msc_pregcomp_ex(rule->ruleset->mp, pattern, PCRE_DOTALL | PCRE_DOLLAR_ENDONLY, 
                    &errptr, &erroffset, msc_pcre_match_limit, msc_pcre_match_limit_recursion);
        if (regex == NULL) {
...
static int msre_op_rx_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    if ((rc == PCRE_ERROR_MATCHLIMIT) || (rc == PCRE_ERROR_RECURSIONLIMIT)) {
        ...
        *error_msg = apr_psprintf(msr->mp, "Regex execution failed (%d): %s", rc, my_error_msg);
        return -1;
    }
    ...
    if (capture && rc > 0) {
        ...
        for(i = 0; i < rc; i++) {
            ...
            s->name_len = strlen(s->name);
            s->value = apr_pstrmemdup(msr->mp, target + ovector[2 * i], ovector[2 * i + 1] - ovector[2 * i]);
    ...
    if (rc != PCRE_ERROR_NOMATCH) { /* Match. */
        ...
        if (strlen(pattern) > 252) {
            *error_msg = apr_psprintf(msr->mp, "Pattern match \"%.252s ...\" at %s.", pattern, var->name);
        } else {
            *error_msg = apr_psprintf(msr->mp, "Pattern match \"%s\" at %s.", pattern, var->name);
        }
...
static int msre_op_pm_param_init(msre_rule *rule, char **error_msg) 
{
    ...
    if (p == NULL) {
        *error_msg = apr_psprintf(rule->ruleset->mp, "Acmp create failed.");
        return 0;
    }
    ...
    for (;;) {
        while((apr_isspace(*phrase) != 0) && (*phrase != '\0')) {
            phrase++;
        }
        if (*phrase == '\0') {
            break;
        ...
        if (strlen(phrase) >= 4 && *phrase == '\\' && *(phrase + 1) == '\'') {
            next = phrase + 2;
            next = strstr(next, "\\'");
            if (next != NULL) {
                if (*(next + 2) == ' ' || *(next + 2) == '\0') {
        ...
        next = phrase;
        while((apr_isspace(*next) == 0) && (*next != 0)) {
            next++;
        }
        acmp_add_pattern(p, phrase, NULL, NULL, next - phrase);
        phrase = next;
...
static int msre_op_pmFromFile_param_init(msre_rule *rule, char **error_msg) 
{
    ...
    rulefile_path = apr_pstrndup(rule->ruleset->mp, rule->filename, 
    ...
    for (;;) {
        ...
        if (rc != APR_SUCCESS) {
            *error_msg = apr_psprintf(rule->ruleset->mp, "Could not open phrase file \"%s\": %s", 
        ...
        for(;;) {
            ...
            if (rc != APR_SUCCESS) {
                *error_msg = apr_psprintf(rule->ruleset->mp, "Could not read \"%s\" line %d: %s", 
                                fn, line, apr_strerror(rc, errstr, 1024));
                return 0;
            }
            ...
            while ((end > start) && (apr_isspace(*end) != 0)) {
                end--;
            }
            if (end > start) {
                *(++end) = '\0';
...
static int msre_op_pm_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg) 
{
    ...
    /* Nothing to read */
    if ((var->value == NULL) || (var->value_len == 0)) {
        return 0;
    }
    ...
    if (rc) {
        ...
        if (strlen(match_escaped) > 252) {
            ...
            *error_msg = apr_psprintf(msr->mp, "Matched phrase \"%s\" at %s.", match_escaped, 
                            var->name);
        }
...
static int msre_op_within_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char ** error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    pattern = apr_pstrdup(msr->mp, match);
    ...
    if (target_length == 0) {
        /* Match. */
        *error_msg = apr_psprintf(msr->mp, "String match within \"\" at %s.", var->name);
        return 1;
    }
     ...
     */
    for (arg = apr_strtok(pattern, " ", &tok); arg; arg = apr_strtok(NULL, " ", &tok)) {
        ...
        if (target[i] == match[0]) {
            ...
            if ((match_length == 1) 
                ...
                *error_msg = apr_psprintf(msr->mp, "String match \"%s\" at %s.",
                        log_escape_ex(msr->mp, match, match_length), var->name);
                return 1;
            }
        }
    }
            ...
            char **error_msg)
{
    ...
    if ((msr->xml == NULL)||(msr->xml->doc == NULL)) {
        *error_msg = apr_psprintf(msr->mp, "XML document tree could not be found for schema validation.");
        return -1;
    }
    ...
    if (msr->msc_reqbody_error) {
        *error_msg = apr_psprintf(msr->mp,
                "XML: Schema validation could not proceed due to previous processing errors.");
        return 1;
    }
...
static int msre_op_verifyCC_init(msre_rule *rule, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    /* Compile rule->op_param */
    regex = msc_pregcomp_ex(rule->ruleset->mp, rule->op_param, PCRE_DOTALL | PCRE_MULTILINE, 
                &errptr, &erroffset, msc_pcre_match_limit, msc_pcre_match_limit_recursion);
    if (regex == NULL) {
        *error_msg = apr_psprintf(rule->ruleset->mp, "Error compiling pattern (offset %d): %s",
                erroffset, errptr);
    ...
    char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (geo == NULL) {
        msr_log(msr, 1, "Geo lookup for \"%s\" attempted without a database.  Set SecGeoLookupDB.", 
            log_escape(msr->mp, geo_host));
        return 0;
    }
    ...
    if (rc <= 0) {
        if (! *error_msg) {
            *error_msg = apr_psprintf(msr->mp, "Geo lookup for \"%s\" failed at %s.", 
    ...
    if (msr->txcfg->debuglog_level >= 9) {
        msr_log(msr, 9, "GEO: %s={country_code=%s, country_code3=%s, country_name=%s, "
                "country_continent=%s, region=%s, city=%s, postal_code=%s, latitude=%f, "
                "longitude=%f, dma_code=%d, area_code=%d}",
                geo_host,
                rec.country_code,
                rec.country_code3,
                rec.country_name,
...
static int msre_op_validateByteRange_init(msre_rule *rule, char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if ((data == NULL)||(rule->op_param_data == NULL)) {
        return -1;
    }
            ...
            char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (table == NULL) {
        *error_msg = apr_psprintf(msr->mp, "Internal Error: validateByteRange table not initialised.");
        return -1;
    }
    ...
    *error_msg = apr_psprintf(msr->mp, "Found %d byte(s) in %s outside range: %s.",count, var->name, 
                    rule->op_param);
            ...
            char **error_msg)
{
    ...
    case 1 :
        ...
        *error_msg = apr_psprintf(msr->mp, "Valid URL Encoding at %s.", var->name);
        break;
    case -2 :
        *error_msg = apr_psprintf(msr->mp, "Invalid URL Encoding: Non-hexadecimal digits used "
                        "at %s.", var->name);
        return 1; /* Invalid match. */
        break;
    case -3 :
        *error_msg = apr_psprintf(msr->mp, "Invalid URL Encoding: Not enough characters at the end "
                        "of input at %s.", var->name);
        return 1; /* Invalid match. */
        break;
    case -1 :
    default :
        *error_msg = apr_psprintf(msr->mp, "Invalid URL Encoding: Internal Error (rc = %d) at %s", 
                        rc, var->name);
        return -1;
        break;
    }
    ...
    for(i = 0; i < var->value_len;) {
        ...
        case UNICODE_ERROR_CHARACTERS_MISSING :
            *error_msg = apr_psprintf(msr->mp, "Invalid UTF-8 encoding: not enough bytes in "
                            "character at %s. [offset \"%d\"]", var->name, i);
            return 1;
            break;
        case UNICODE_ERROR_INVALID_ENCODING :
            *error_msg = apr_psprintf(msr->mp, "Invalid UTF-8 encoding: invalid byte value in "
                            "character at %s. [offset \"%d\"]", var->name, i);
            return 1;
            break;
        case UNICODE_ERROR_OVERLONG_CHARACTER :
            *error_msg = apr_psprintf(msr->mp, "Invalid UTF-8 encoding: overlong character detected "
        ...
        case UNICODE_ERROR_DECODING_ERROR :
            *error_msg = apr_psprintf(msr->mp, "Error validating UTF-8 decoding at %s. [offset \"%d\"]", 
                            var->name, i);
            return 1;
            break;
        }
        ...
        if (rc <= 0) {
            *error_msg = apr_psprintf(msr->mp, "Internal error during UTF-8 validation at %s.", var->name);
            return 1;
        }
...
/* eq */
static int msre_op_eq_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    int left, right;
    char *target = NULL;
...
/* gt */
static int msre_op_gt_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    int left, right;
    char *target = NULL;
...
/* lt */
static int msre_op_lt_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    int left, right;
    char *target = NULL;
...
/* ge */
static int msre_op_ge_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    int left, right;
    char *target = NULL;
...
/* le */
static int msre_op_le_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    int left, right;
    char *target = NULL;
...
static void search_cookie_name(modsec_rec *msr, apr_table_t *cookiename_tb, int cookiename_num)
{
    ...
    set_cookies_arr = apr_table_elts(msr->request_cookies);
    ...
    for (i = 0; i < set_cookies_arr->nelts; i++) {
        ...
        for (k = 0; k < cookiename_arr->nelts; k++) {
            ...
            if (cookie_attr != NULL) {
                cookiename_te[k].val = (void *)cookie_attr->attrval;
...
/* 清除和请求cookie有关的所有变量 */
static void clear_cookie_cache(modsec_rec *msr)
{
    apr_table_unset(msr->request_headers, "Cookie");
    apr_table_unset(msr->r->headers_in, "Cookie");
    apr_table_clear(msr->request_cookies);
    apr_hash_clear(msr->var_fetch_cache);
}

static int requestcookie_change_syn(modsec_rec *msr, apr_table_t *tb, apr_table_t *cookies_tb, int locate)
{
    ...
    for (j = 0; j < attrsarr->nelts; j++) {
        httphead = (char *)attrste[j].key;
        if (strcasecmp(httphead, "Cookie") == 0) {
            if (k == locate) {
                if (apr_is_empty_table(cookies_tb)) {
                    attrste[j].val = "";
                } else {
...
static int msre_op_verifyCookieSignature_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (apr_is_empty_table(msr->request_cookies)) {
        /* 没有cookie直接退出 */
        return 0;
    }
    
    /* 在cookie中查找"xwrj_signame"字段 */
    ori_signature = NULL;
    set_cookies_arr = apr_table_elts(msr->request_cookies);
    set_cookies_te = (apr_table_entry_t *)set_cookies_arr->elts;
    for (signame_locate = 0; signame_locate < set_cookies_arr->nelts; signame_locate++) {
        cookies_tb = (apr_table_t *)set_cookies_te[signame_locate].val;
        cookie_attr = (msre_cookie_attr_t *)apr_table_get(cookies_tb, "xwrj_signame");
        if (cookie_attr != NULL) {
            ori_signature = cookie_attr->attrval;
            break;
        }
    }

    if (ori_signature == NULL) {
        /* 没有签名字段，不能匹配，删除cookie */
        msr_log(msr, 9, "verifyCookieSignature: can't find 'xwrj_signame'");
        clear_cookie_cache(msr);
        return 0;
    }

    cookie_name_tb = apr_table_make(msr->mp, 8);
    if (cookie_name_tb == NULL) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieSignature: cookiename table is null");
        return -1;
    }

    /* 从操作参数中解析出要进行校验的cookie名 */
    cookiename_num = parse_cookie_name(msr, rule->op_param, cookie_name_tb);
    if (cookiename_num < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieSignature: parse cookiename error");
        return -1;
    }

    /* 从request-cookie头域中查找cookie名所对应的value值,
     * cookiename_num为0时，计算签名只是用cookie关键字 
     */
    if (cookiename_num > 0) {
    ...
    if (signature == NULL) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieSignature: generate signature error");
        return -1;
    }
    
    /* 签名不匹配，删除cookie */
    if (strcmp(ori_signature, signature) != 0) {  
        clear_cookie_cache(msr);
        return 0;
    }

    /* 匹配成功，删除cookie中的签名字段 */
    apr_table_unset(cookies_tb, "xwrj_signame");
    /* set-cookie已改变，同步到请求头 */     
    if (requestcookie_change_syn(msr, msr->r->headers_in, cookies_tb, signame_locate) < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: requestcookie_change_syn error");
        return -1;        
    }
    if (requestcookie_change_syn(msr, msr->request_headers, cookies_tb, signame_locate) < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: requestcookie_change_syn error");
        return -1; 
    }
    apr_hash_clear(msr->var_fetch_cache);
    
    *error_msg = apr_psprintf(msr->mp, "Operator verifyCookieSignature matched %s at %s.",
            var->value, var->name);
...
static int msre_op_verifyCookieIpSignature_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    const apr_array_header_t *set_cookies_arr;
    apr_table_entry_t *set_cookies_te;
    apr_table_t *cookies_tb;
    ...
    *error_msg = NULL;
    ...
    if (ori_signature == NULL) {
        /* 没有签名字段，不能匹配，删除cookie */
        msr_log(msr, 9,  "verifyCookieIpSignature: can't find 'xwrj_signame'");
        clear_cookie_cache(msr);
        return 0;
    }
    ...
    cookie_name_tb = apr_table_make(msr->mp, 8);
    if (cookie_name_tb == NULL) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: cookiename table is null");
        return -1;;
    }
    ...
    cookiename_num = parse_cookie_name(msr, rule->op_param, cookie_name_tb);
    if (cookiename_num < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: parse cookiename error");
        return -1;
    }
    ...
    /* 获取cookie签名，计算签名的时候要用ip地址 */
    signature = get_cookie_signature(msr, cookie_name_tb, 1);
    if (signature == NULL) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: generate signature error");
        return -1;
    }
    ...
    /* set-cookie已改变，同步到请求头 */  
    if (requestcookie_change_syn(msr, msr->r->headers_in, cookies_tb, signame_locate) < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: generate setcookiestr1 error");
        return -1;        
    }
    if (requestcookie_change_syn(msr, msr->request_headers, cookies_tb, signame_locate) < 0) {
        *error_msg = apr_psprintf(msr->mp, "verifyCookieIpSignature: generate setcookiestr2 error");
        return -1; 
    }
    apr_hash_clear(msr->var_fetch_cache);
    
    *error_msg = apr_psprintf(msr->mp, "Operator verifyCookieIpSignature matched %s at %s.",
            var->value, var->name);

    return 1;
}

static void decryptcookie_change_syn(modsec_rec *msr, apr_table_t *tb, apr_table_t *cookie,  int locate)
{
    const apr_array_header_t *attrsarr = NULL;
    apr_table_entry_t *attrste = NULL;
    int j, k;
...
/* decryptCookie */
static int msre_op_decryptCookie_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    ...
    *error_msg = NULL;
    ...
    num = parse_cookie_name(msr, str.value, attrnames);
    if (num < 0) {
        return 0;
    }
    
    if (apr_is_empty_table(attrnames)) {
        return 1;
    }
    
    cookiesarr = apr_table_elts(msr->request_cookies);
    ...
    for (i = 0; i < cookiesarr->nelts; i++) {
        ...
        for (j = 0; j < attrsarr->nelts; j++) {
            ...
            if (attr != NULL) {
                ptemp0 = msc_tripleDes(msr->mp, (unsigned char*)attr->attrval, get_3des_key(msr), get_3des_vector(msr), DES_DECRYPT);
                if (ptemp0 == NULL) {
                    msr_log(msr, 1, "Failed to DECRYPT: %s", rule->op_param);
                    clear_cookie_cache(msr);
                    return 0;
                }
        ...
        if (setcookie_modified) {
            /*cookie域发生改变，修改请求头*/
            decryptcookie_change_syn(msr, msr->r->headers_in, cookie, i);
            decryptcookie_change_syn(msr, msr->request_headers, cookie, i);
        }
    }
    ...
    if (msr->txcfg->debuglog_level >= 4) {
         msr_log(msr, 4, "DECRYPT Cookie attribute\"%s\" ",
...
/* decryptIpCookie */
static int msre_op_decryptIpCookie_execute(modsec_rec *msr, msre_rule *rule, msre_var *var, char **error_msg)
{
    msc_string str;
    const apr_array_header_t *cookiesarr = NULL;
    apr_table_entry_t *cookieste = NULL;
    ...
    *error_msg = NULL;
    ...
    for (i = 0; i < cookiesarr->nelts; i++) {
        ...
        for (j = 0; j < attrsarr->nelts; j++) {
            ...
            msre_cookie_attr_t *attr = (msre_cookie_attr_t *)apr_table_get(cookie, attrname);
            if (attr != NULL) {
                ptemp0 = (char *)msc_tripleDes(msr->mp, (unsigned char*)attr->attrval, get_3des_key(msr), get_3des_vector(msr), DES_DECRYPT);
                if (ptemp0 == NULL) {
                    msr_log(msr, 1, "Failed to DECRYPT: %s", rule->op_param);
                    clear_cookie_cache(msr);
                    return 0;
                ...
                /*效验IP*/
                ptemp1 = strrchr(ptemp0, '|');
                if (ptemp1 != NULL) {
                    *ptemp1 = 0;
                    ptemp1++;
                    if (strncasecmp(ptemp1, msr->remote_addr, strlen(msr->remote_addr)) == 0) {
                        attr->attrval = ptemp0;
                        setcookie_modified = 1;
                        change = 1;
                    } else {
                        msr_log(msr, 1, "Failed to DECRYPT ip: %s", rule->op_param);
                        clear_cookie_cache(msr);
                        return 0;
                    }
                }
    ...
    if (msr->txcfg->debuglog_level >= 4) {
         msr_log(msr, 4, "DECRYPT ip Cookie attribute\"%s\" ",
                    log_escape(msr->mp,  var->value));
    }
...
void msre_engine_register_default_operators(msre_engine *engine)
{
   ...
   /* verifyCookieSignature */
    msre_engine_op_register(engine,
        "verifyCookieSignature",
        NULL,
        msre_op_verifyCookieSignature_execute
    );
    ...
    msre_engine_op_register(engine,
        "verifyCookieIpSignature",
        NULL,
        msre_op_verifyCookieIpSignature_execute
    );
    ...
    msre_engine_op_register(engine,
        "decryptIpCookie",
        NULL,
        msre_op_decryptIpCookie_execute
    );
}
