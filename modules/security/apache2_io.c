...
#define syslog_print_info(module, mnemonic, fmt, args...) \
    do {\
        openlog((module "/6/" mnemonic), LOG_CONS, 0);\
 ...
 * processing module).
 */
apr_status_t input_filter(ap_filter_t *f, apr_bucket_brigade *bb_out, ap_input_mode_t mode, 
                apr_read_type_e block, apr_off_t nbytes)
{
    ...
    if (msr == NULL) {
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, f->r->server, "ModSecurity: Internal"
            " error in input filter: msr is null.");
    ...
    if (msr->phase < PHASE_REQUEST_BODY) {
        msr_log(msr, 1, "Internal error: REQUEST_BODY phase incomplete for input filter in phase %d", 
            msr->phase);
        return APR_EGENERAL;
    }
    ...
    if ((msr->if_status == IF_STATUS_COMPLETE) || (msr->if_status == IF_STATUS_NONE)) {
        if (msr->txcfg->debuglog_level >= 4) {
            msr_log(msr, 4, "Input filter: Input forwarding already complete, skipping "
                "(f %pp, r %pp).", f, f->r);
        }
        ap_remove_input_filter(f);
    ...
    if (chunk && (!msr->txcfg->stream_inbody_inspection 
            || (msr->txcfg->stream_inbody_inspection && msr->if_stream_changed == 0))) {
        /* Copy the data we received in the chunk */
        bucket = apr_bucket_heap_create(chunk->data, chunk->length, NULL, f->r->connection->bucket_alloc);
        if (bucket == NULL) {
            return APR_EGENERAL;
        }
        ...
        if (msr->txcfg->debuglog_level >= 4) {
            msr_log(msr, 4, "Input stream filter: Forwarded %" APR_SIZE_T_FMT " bytes.", 
    ...
    /* 计算请求体大小 */
    apr_brigade_length(bb_out, 1, &length);
    msr->r->request_bytes += length;
    ...
    *error_msg = NULL;
    ...
    do {
        ...
        if (rc != APR_SUCCESS) {
            ...
            switch(rc) {
            case APR_EOF :
                *error_msg = apr_psprintf(msr->mp, "Error reading request body: %s", 
            ...
            case AP_FILTER_ERROR :
                *error_msg = apr_psprintf(msr->mp, "Error reading request body: HTTP Error "
                                "413 - Request entity too large. (Most likely.)");
                return -3;
            case APR_EGENERAL :
        ...
        /* Loop through the buckets in the brigade in order to extract the size of the data available. */
        for(bucket = APR_BRIGADE_FIRST(bb_in); 
            ...
            if (rc != APR_SUCCESS) {
                *error_msg = apr_psprintf(msr->mp, "Failed reading input / bucket (%d): %s", 
            ...
            if (msr->reqbody_length + buflen > (apr_size_t)msr->txcfg->reqbody_limit) {
                if((msr->txcfg->is_enabled == MODSEC_ENABLED) 
                    ...
                    *error_msg = apr_psprintf(msr->mp, "Request body is larger than the configured"
                                    " limit (%ld).", msr->txcfg->reqbody_limit);
                    return -5;
            ...
            if (msr->txcfg->stream_inbody_inspection == 1) {
                ...
                if (msr->stream_input_length > (apr_size_t)msr->txcfg->reqbody_inmemory_limit) {
                    if (msr->stream_input_data != NULL) {
                        free(msr->stream_input_data);
                        msr->stream_input_data = NULL;
                        msr->stream_input_length = 0;
                    }
                    syslog_print_info("SECURITY", "ADMIN", "The input flow[%s-%s] exceeds the memory "
                            "limit[%ld], not content to replace.", msr->local_addr,msr->remote_addr,
...
static int output_filter_should_run(modsec_rec *msr, request_rec *r) 
{
    ...
    if (r->content_type != NULL) {
        ...
        if (strcmp(content_type, "text/html") == 0) {
            /* Useful information to have should we later decide to do something with the HTML output. */
    ...
    if (apr_table_get(msr->txcfg->of_mime_types, content_type) != NULL) {
        return 1;
    }

    msr_log(msr, 4, "Output filter: Not buffering response body for unconfigured MIME "
        "type \"%s\".", content_type);
 ...
 */
static apr_status_t output_filter_init(modsec_rec *msr, ap_filter_t *f, apr_bucket_brigade *bb_in)
{
    request_rec *r = f->r;
    ...
    if (rc < 0) {
        return -1; /* output_filter_should_run() generates error msg */
    }
    if (rc == 0) {
        return 0;
    }

    /* Do not check the output limit if we are willing to process partial response bodies. */
    if (msr->txcfg->is_enabled == MODSEC_DETECTION_ONLY) {
        return 1;
    }
...
static void inject_content_to_of_brigade(modsec_rec *msr, ap_filter_t *f) 
{
    ...
    if (msr->txcfg->content_injection_enabled && msr->stream_output_data != NULL) {
        ...
        bucket_ci = apr_bucket_heap_create(msr->stream_output_data, msr->stream_output_length, NULL, 
                        f->r->connection->bucket_alloc);

        for (b = APR_BRIGADE_FIRST(msr->of_brigade); 
             b != APR_BRIGADE_SENTINEL(msr->of_brigade); 
             b = APR_BUCKET_NEXT(b))  {
        ...
        if (msr->txcfg->debuglog_level >= 9) {
            msr_log(msr, 9, "Content Injection: Data reinjected bytes [%"APR_SIZE_T_FMT"]",
...
static void prepend_content_to_of_brigade(modsec_rec *msr, ap_filter_t *f) 
{
    if ((msr->txcfg->content_injection_enabled) && (msr->content_prepend) && (!msr->of_skipping)) {
        ...
        bucket_ci = apr_bucket_heap_create(msr->content_prepend, msr->content_prepend_len, NULL, 
                        f->r->connection->bucket_alloc);
...
static int flatten_response_body(modsec_rec *msr) 
{
    ...
    if (msr->resbody_length + 1 <= 0) {
        msr_log(msr, 1, "Output filter: Invalid response length: %" APR_SIZE_T_FMT, 
            msr->resbody_length);
        return -1;
    }
	

	/*内存限定起作用的情况是，小于响应体大小限定值，小于resbody_length值*/
    partial = 0;
	if (msr->txcfg->of_inmemory_limit < msr->txcfg->of_limit) {
        if (msr->resbody_length > msr->txcfg->of_inmemory_limit) {
            partial = 1;
            msr->resbody_length = msr->txcfg->of_inmemory_limit;
				msr_log(msr, 4, "Output filter: Processing partial response body (memory limit %ld)",
                            msr->txcfg->of_inmemory_limit);
        }
    }
    msr->resbody_data = apr_palloc(msr->mp, msr->resbody_length + 1);
    if (msr->resbody_data == NULL) {
        msr_log(msr, 1, "Output filter: Response body data memory allocation failed. Asked for: %" 
            APR_SIZE_T_FMT, msr->resbody_length + 1);
        return -1;
    }
        ...
        if (!partial) {
            if(msr->stream_output_data == NULL) {
                ...
                if(stream_output_body == NULL)  {
                    ...
                    msr_log(msr, 1, "Output filter: Stream Response body data memory allocation "
                        "failed. Asked for: %" APR_SIZE_T_FMT, msr->stream_output_length + 1);
                    return -1;
                }
            ...
            if (msr->stream_output_data == NULL) {
                msr_log(msr, 1, "Output filter: Stream Response body data memory allocation failed. "
                    "Asked for: %" APR_SIZE_T_FMT, msr->stream_output_length + 1);
                return -1;
            }
            ...
            syslog_print_info("SECURITY", "ADMIN", "The output flow[%s-%s] exceeds the memory "
                    "limit[%ld], not content to replace.", msr->local_addr,msr->remote_addr,
                    msr->txcfg->of_inmemory_limit);
        }
    }
...
static int parse_response_cookies(modsec_rec *msr)
{
    ...
    msr->response_cookies = apr_table_make(msr->mp, 8);
    if (msr->response_cookies == NULL) {
        return -1;
    }

    arr = apr_table_elts(msr->r->headers_out);
    ...
    for (i = 0, j = 0; i < arr->nelts; i++) {
        ...
        snprintf(key, 20, "cookies%d", ++j);
        cookies_key = apr_pstrdup(msr->mp, key);
...
apr_status_t output_filter(ap_filter_t *f, apr_bucket_brigade *bb_in) 
{
    ...
    if (msr == NULL) {
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, f->r->server, "ModSecurity: Internal"
            " Error: msr is null in output filter.");
    ...
    /* 计算响应体大小:cache模块和security模块只会统计一次 */
    apr_brigade_length(bb_in, 1, &length);
    msr->r->response_bytes += length;

    /* Check if it can skip the rule check */
    if (msr->white_list 
            || (msr->txcfg->is_enabled == MODSEC_ENABLED && msr->black_list_flag)) {
        if (msr->txcfg->debuglog_level >= 4) {
            if (msr->white_list) {
                msr_log(msr, 4, "Skipping respone phase rule check.");
            }
        }
    ...
    /* Put back the Accept-Encoding and TE request headers if they were removed from the request. */
    ...
    } else if (msr->of_status == OF_STATUS_COMPLETE) {
        msr_log(msr, 1, "Output filter: Internal error: output filtering complete yet filter "
            "was invoked.");
        ap_remove_output_filter(f);
        return APR_EGENERAL;
    }
    ...
    for(bucket = APR_BRIGADE_FIRST(bb_in); 
        ...
        /* Look into response data if configured to do so, unless we've already processed a partial 
         * response.
         */
        if ((msr->of_skipping == 0) && (!msr->of_partial)) { /* Observe the response data. */
            ...
            if (rc != APR_SUCCESS) {
                ...
                msr_log(msr, 1, "Output filter: Failed to read bucket (rc %d): %s", rc, 
            ...
            if (msr->resbody_length + buflen > (apr_size_t)msr->txcfg->of_limit) {
                /* The size of the response is larger than what we're ready to accept. We need to 
        ...
        if (APR_BUCKET_IS_EOS(bucket)) {
            ...
            if (       (msr->txcfg->content_injection_enabled) 
                    && (msr->content_append)
                    && (msr->of_skipping || msr->of_partial || start_skipping)) {
                apr_bucket *bucket_ci = NULL;

                bucket_ci = apr_bucket_heap_create(msr->content_append, msr->content_append_len, NULL, 
                                f->r->connection->bucket_alloc);
    ...
    /* Add buckets in this brigade to the brigade we have in the context, but only if we actually
    ...
    if ((msr->of_skipping == 0) && (msr->of_partial == 0)) {
        ...
        if (msr->txcfg->debuglog_level >= 4) {
            msr_log(msr, 4, "Output filter: Completed receiving response body (buffered "
                "%s - %" APR_SIZE_T_FMT " bytes).", (msr->of_partial ? "partial" : "full"), 
                msr->resbody_length);
        }
    ...
    /* Process phase RESPONSE_BODY, but only if it hasn't been processed already. */
    ...
    /* Now send data down the filter stream (full-buffering only).*/
    if ((msr->of_skipping == 0) && (!msr->of_partial)) {
        ...
        if ((msr->txcfg->content_injection_enabled) && (msr->content_append)) {
            ...
            bucket_ci = apr_bucket_heap_create(msr->content_append, msr->content_append_len, 
