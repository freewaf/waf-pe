...
static char *var_generic_list_validate(msre_ruleset *ruleset, msre_var *var) 
{
    ...
    if ((strlen(var->param) > 2) 
            && (var->param[0] == '/') 
            && (var->param[strlen(var->param) - 1] == '/')) { /* Regex. */
        ...
        regex = msc_pregcomp(ruleset->mp, pattern, PCRE_DOTALL | PCRE_CASELESS | PCRE_DOLLAR_ENDONLY, 
                    &errptr, &erroffset);
        if (regex == NULL) {
            return apr_psprintf(ruleset->mp, "Error compiling pattern (offset %d): %s", erroffset,
                    errptr);
        }
...
/* ARGS */
static int var_args_generate(modsec_rec *msr, msre_var *var, msre_rule *rule, apr_table_t *vartab, 
            apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (var->param == NULL) {
            match = 1; /* Unconditional inclusion. */
        } else {
            if (var->param_data != NULL) { /* Regex. */
                ...
                if (!(msc_regexec((msc_regex_t *)var->param_data, arg->name,
                    arg->name_len, &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
                if (strcasecmp(arg->name, var->param) == 0) {
                    match = 1;
                }
            }
        }
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (strcmp("QUERY_STRING", arg->origin) != 0) {
            continue;
        }
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                char *my_error_msg = NULL;
                /* Run the regex against the argument name. */
                if (!(msc_regexec((msc_regex_t *)var->param_data, arg->name, arg->name_len,
        ...
        if (match) {
            ...
            rvar->value = arg->value;
            rvar->value_len = arg->value_len;
            rvar->name = apr_psprintf(mptmp, "ARGS_GET:%s", log_escape_nq_ex(mptmp, arg->name,
                    arg->name_len));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value = arg->name;
            rvar->value_len = arg->name_len;
            rvar->name = apr_psprintf(mptmp, "ARGS_POST_NAMES:%s", log_escape_nq_ex(mptmp, 
                            arg->name, arg->name_len));
...
static char *var_env_validate(msre_ruleset *ruleset, msre_var *var) 
{
    ...
    if ((strlen(var->param) > 2) && (var->param[0] == '/') &&(var->param[strlen(var->param) - 1] == '/')) {
...
static int var_env_generate(modsec_rec *msr, msre_var *var, msre_rule *rule, apr_table_t *vartab, 
            apr_pool_t *mptmp)
{
...
/* THE_REQUEST */
static int var_request_uri_flag_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp, msr->r->request_uri_flag);
}
...
static int var_remote_host_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    const char *value1 = ap_get_remote_host(msr->r->connection, msr->r->per_dir_config, REMOTE_NAME,
            NULL);
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                ...
                if (!(msc_regexec((msc_regex_t *)var->param_data, str->name,
                        str->name_len, &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
                if (strcasecmp(str->name, var->param) == 0) {
                    match = 1;
                }
            }
        }
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                char *my_error_msg = NULL;
                if (!(msc_regexec((msc_regex_t *)var->param_data, str->name, str->name_len,
                        &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
...
static int var_highest_severity_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    return var_simple_generate(var, vartab, mptmp, apr_psprintf(mptmp, "%d", msr->highest_severity));
}
...
static int var_matched_var_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    return var_simple_generate_ex(var, vartab, mptmp, apr_pmemdup(mptmp, msr->matched_var->value,
                                      msr->matched_var->value_len), msr->matched_var->value_len);
}
...
static int var_matched_var_name_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    return var_simple_generate_ex(var, vartab, mptmp, apr_pmemdup(mptmp, msr->matched_var->name,
                                      msr->matched_var->name_len), msr->matched_var->name_len);
}
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if ((parts[i]->type == MULTIPART_FILE) && (parts[i]->tmp_file_name != NULL)) {
            ...
            if (var->param == NULL) {
                ...
                if (var->param_data != NULL) { /* Regex. */
                    ...
                    if (!(msc_regexec((msc_regex_t *)var->param_data, parts[i]->name,
                            strlen(parts[i]->name), &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                        match = 1;
                    }
                } else { /* Simple comparison. */
            ...
            if (match) {
                ...
                rvar->value_len = strlen(rvar->value);
                rvar->name = apr_psprintf(mptmp, "FILES_TMPNAMES:%s", log_escape_nq(mptmp, parts[i]->name));
                apr_table_addn(vartab, rvar->name, (void *)rvar);
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FILE) {
            ...
            if (match) {
                ...
                rvar->value_len = strlen(rvar->value);
                rvar->name = apr_psprintf(mptmp, "FILES:%s", log_escape_nq(mptmp, parts[i]->name));
                apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for(i = 0; i < msr->mpd->parts->nelts; i++) {
        if (parts[i]->type == MULTIPART_FILE) {
            ...
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "FILES_NAMES:%s", log_escape_nq_ex(mptmp, parts[i]->name, rvar->value_len));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    rvar = apr_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = apr_psprintf(mptmp, "%" APR_TIME_T_FMT, (apr_time_msec(apr_time_now() - msr->r->request_time)));
    rvar->value_len = strlen(rvar->value);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    tm = localtime(&tc);
    rvar = apr_pmemdup(mptmp, var, sizeof(msre_var));
    rvar->value = apr_psprintf(mptmp, "%02d%02d", (tm->tm_year / 100) + 19, tm->tm_year % 100);
    rvar->value_len = strlen(rvar->value);
    apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    if (msr->msc_reqbody_buffer != NULL) {
        return var_simple_generate_ex(var, vartab, mptmp, msr->msc_reqbody_buffer,
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                char *my_error_msg = NULL;
                if (!(msc_regexec((msc_regex_t *)var->param_data, str->name,
                        strlen(str->name), &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
                if (strcasecmp(str->name, var->param) == 0) {
        ...
        /* If we had a match add this argument to the collection. */
        if (match && (strncmp(str->name,"MATCHED_VARS:",13) != 0) &&
                (strncmp(str->name,"MATCHED_VARS_NAMES:",19))) {
            msre_var *rvar = apr_palloc(mptmp, sizeof(msre_var));

            rvar->value = apr_pstrndup(mptmp, str->name, strlen(str->name));
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "MATCHED_VARS_NAMES:%s", log_escape_nq(mptmp, str->name));
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match && (strncmp(str->name,"MATCHED_VARS:",13) != 0) && (strncmp(str->name,"MATCHED_VARS_NAMES:",19))) {
            ...
            rvar->value_len = str->value_len;
            rvar->name = apr_psprintf(mptmp, "MATCHED_VARS:%s", log_escape_nq(mptmp, str->name));
            ...
            if (msr->txcfg->debuglog_level >= 9) {
                msr_log(msr, 9, "Set variable \"%s\" value \"%s\" size %d to collection.", 
                    rvar->name, rvar->value, rvar->value_len);
            }
...
static int var_request_cookies_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    const apr_array_header_t *arr = NULL, *cookies_arr = NULL;
    const apr_table_entry_t *te = NULL, *cookies_te = NULL;
    apr_table_t *cookies;
    msre_cookie_attr_t *attr_val;
    int i, j, count = 0;

    arr = apr_table_elts(msr->request_cookies);
    te = (apr_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        cookies = (apr_table_t *)te[i].val;
        cookies_arr = apr_table_elts(cookies);
        cookies_te = (apr_table_entry_t *)cookies_arr->elts;
        for (j = 0; j < cookies_arr->nelts; j++) {
            int match = 0;
            attr_val = (msre_cookie_attr_t *)cookies_te[j].val;
            ...
            if (var->param == NULL) {
                ...
                if (var->param_data != NULL) { /* Regex. */
                    char *my_error_msg = NULL;
                    if (!(msc_regexec((msc_regex_t *)var->param_data, cookies_te[j].key, strlen(cookies_te[j].key),
                            &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                        match = 1;
                    }
                } else { /* Simple comparison. */
                    if (strcasecmp(cookies_te[j].key, var->param) == 0) {
                        match = 1;
                    }
                }
            }
            ...
            if (match) {
                ...
                rvar->value = attr_val->attrval;
                if (rvar->value == NULL) {
                    rvar->value_len = 0;
                } else {
                    rvar->value_len = strlen(rvar->value);
                }
                rvar->name = apr_psprintf(mptmp, "REQUEST_COOKIES:%s", log_escape_nq(mptmp, cookies_te[j].key));
                apr_table_addn(vartab, rvar->name, (void *)rvar);
                count++;
            }
        }
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        for (j = 0; j < cookies_arr->nelts; j++) {
            ...
            if (match) {
                ...
                rvar->value = cookies_te[j].key;
                rvar->value_len = strlen(rvar->value);
                rvar->name = apr_psprintf(mptmp, "REQUEST_COOKIES_NAMES:%s", log_escape_nq(mptmp, cookies_te[j].key));
                apr_table_addn(vartab, rvar->name, (void *)rvar);
                count++;
            }
        }
...
/* REQUEST_COOKIES_BLOCK */
static int var_request_cookies_block_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    cookies_block = NULL;
    arr = apr_table_elts(msr->r->headers_in);
    te = (apr_table_entry_t *)arr->elts;
    for (i = 0; i < arr->nelts; i++) {
        if (strcasecmp(te[i].key, "Cookie") == 0 && te[i].val != NULL) {
            cookies_block = apr_pstrcat(msr->mp, cookies_block ? cookies_block : "",
                te[i].val, NULL);
            len = strlen(te[i].val);
            value = te[i].val + len - 1;
            while (value >= te[i].val && isspace(*value)) {
                value--;
            }
            if (value >= te[i].val && *value != ';') {
                cookies_block = apr_pstrcat(msr->mp,
                    cookies_block ? cookies_block : "", "; ", NULL);
            }
        }
    }

    if (cookies_block == NULL) {
        return 0;
    }
    ...
    rvar->value = cookies_block;
    rvar->value_len = strlen(cookies_block);
    apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                char *my_error_msg = NULL;
                if (!(msc_regexec((msc_regex_t *)var->param_data, te[i].key, strlen(te[i].key), 
                    &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
                if (strcasecmp(te[i].key, var->param) == 0) {
                    match = 1;
                }
            }
        }
        ...
        if (match) {
            ...
            rvar->value = te[i].val;
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "REQUEST_HEADERS:%s", log_escape_nq(mptmp, te[i].key));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (var->param == NULL) {
            ...
            if (var->param_data != NULL) { /* Regex. */
                ...
                if (!(msc_regexec((msc_regex_t *)var->param_data, te[i].key,
                        strlen(te[i].key), &my_error_msg) == PCRE_ERROR_NOMATCH)) {
                    match = 1;
                }
            } else { /* Simple comparison. */
        ...
        if (match) {
            ...
            rvar->value = te[i].key;
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "REQUEST_HEADERS_NAMES:%s", log_escape_nq(mptmp, te[i].key));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    if (msr->stream_output_data != NULL) {
        return var_simple_generate_ex(var, vartab, mptmp, msr->stream_output_data,
                msr->stream_output_length);
    }
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    if (msr->stream_input_data != NULL) {
        return var_simple_generate_ex(var, vartab, mptmp, msr->stream_input_data, 
                msr->stream_input_length);
    }
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    if (msr->resbody_data != NULL) {
        return var_simple_generate_ex(var, vartab, mptmp, msr->resbody_data, msr->resbody_length);
    }
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value = te[i].val;
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "RESPONSE_HEADERS:%s", log_escape_nq(mptmp, te[i].key));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
            ...
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value = te[i].key;
            rvar->value_len = strlen(rvar->value);
            rvar->name = apr_psprintf(mptmp, "RESPONSE_HEADERS_NAMES:%s", log_escape_nq(mptmp, te[i].key));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
...
/* REFERURL */
static int var_referurl_generate(modsec_rec *msr, msre_var *var, msre_rule *rule,
            apr_table_t *vartab, apr_pool_t *mptmp)
{
    const apr_array_header_t *arr = NULL;
    const apr_table_entry_t *te = NULL;
    ...
    target_col = (apr_table_t *)apr_table_get(msr->collections, "referurl");
    if (target_col == NULL) {
        return 0;
    }
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (match) {
            ...
            rvar->value = str->value;
            rvar->value_len = str->value_len;
            rvar->name = apr_psprintf(mptmp, "REFERURL:%s", log_escape_nq_ex(mptmp, str->name, str->name_len));
            apr_table_addn(vartab, rvar->name, (void *)rvar);
 ...
 *
 */
void msre_engine_variable_register(msre_engine *engine, const char *name, unsigned int type, 
        unsigned int argc_min, unsigned int argc_max, fn_var_validate_t validate, 
        fn_var_generate_t generate, unsigned int is_cacheable, unsigned int availability,
        enum ATTACK_DOMAIN domain)
{
    msre_var_metadata *metadata = (msre_var_metadata *)apr_pcalloc(engine->mp, sizeof(msre_var_metadata));
    if (metadata == NULL) {
        return;
    }
...
void msre_engine_register_default_variables(msre_engine *engine) 
{
    ...
    msre_engine_variable_register(engine,
        ...
        0, 0,
        NULL,
        var_request_cookies_block_generate,
        VAR_CACHE,
        PHASE_REQUEST_HEADERS,
        DOMAIN_COOKIE
    );
