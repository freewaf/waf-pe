...
char *update_rule_target(cmd_parms *cmd, directory_config *dcfg, msre_ruleset *rset, 
...
static void msre_actionset_action_add(msre_actionset *actionset, msre_action *action)
{
     ...
     */
    if (        (actionset->parent_intercept_action_rec != NULL) 
             && (actionset->parent_intercept_action_rec != NOT_SET_P) 
             && (strcmp("block", action->metadata->name) == 0)
             && (strcmp("block", action->metadata->name) == 0)) {
            ...
            modsec_rec *msr, char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if ((var->name != NULL) && (strlen(var->name) > 5) && (strncmp("HTTP_", var->name, 5) == 0)) {
    ...
    if (var->is_counting) {
        if (var->metadata->type == VAR_SIMPLE) {
            *error_msg = apr_psprintf(engine->mp, "The & modificator does not apply to non-collection variables.");
            return NULL;
        }
    }
    ...
    if (varparam == NULL) {
        if (var->metadata->argc_min > 0) {
            *error_msg = apr_psprintf(engine->mp, "Missing mandatory parameter for variable %s.", name);
            return NULL;
        }
    } else { /* Parameter present */
        /* Do we allow a parameter? */
        if (var->metadata->argc_max == 0) {
            *error_msg = apr_psprintf(engine->mp, "Variable %s does not support parameters.", name);
            return NULL;
        }
...
static msre_var *msre_create_var(msre_ruleset *ruleset, const char *name, const char *param,
                    modsec_rec *msr, char **error_msg)
{
    msre_var *var = msre_create_var_ex(ruleset->engine->mp, ruleset->engine, name, param, msr, 
                        error_msg);
    if (var == NULL) {
        return NULL;
    }
...
static int msre_tfns_parse(msre_engine *engine, const char *text, apr_table_t *tfnset, 
               char **error_msg)
{
    const apr_array_header_t *tarr;
    const apr_table_entry_t *telts;
    ...
    for (i = 0; i < tarr->nelts; i++) {
        ...
        metadata = msre_engine_tfn_resolve(engine, telts[i].val);
        if (metadata == NULL) {
            *error_msg = apr_psprintf(engine->mp, "Invalid transformation function: %s", telts[i].val);
            return -1;
        }       

        apr_table_addn(tfnset, telts[i].val, (void *)metadata);
    }
    ...
    *error_msg = NULL;
    ...
    if (param == NULL) { /* Parameter not present */
        if (action->metadata->argc_min > 0) {
            *error_msg = apr_psprintf(engine->mp, "Missing mandatory parameter for action %s", name);
            return NULL;
        }
    } else { /* Parameter present */
        ...
        if ((param[0] == '+')||(param[0] == '-')) {
            if (action->metadata->allow_param_plusminus == 0) {
                *error_msg = apr_psprintf(engine->mp, "Action %s does not allow +/- modificators.", name);
                return NULL;
 ...
 */
int msre_parse_generic(apr_pool_t *mp, const char *text, apr_table_t *vartable, char **error_msg)
{
    char *p = (char *)text;
    int count = 0;
    ...
    *error_msg = NULL;
    ...
    while(*p != '\0') {
        ...
        name = p;
        while((*p != '\0') && (*p != '|') && (*p != ':') && (*p != ',') && (!isspace(*p))) {
            p++; /* ENH replace with isvarnamechar() */
        }
        ...
        if (*p != ':') { /* we don't have a parameter */
            ...
            *error_msg = apr_psprintf(mp, "Unexpected character at position %d: %s", (int)(p - text), text);
            return -1;
        }
 ...
 */
msre_actionset *msre_actionset_create(msre_engine *engine, const char *text, char **error_msg)
{
    msre_actionset *actionset = (msre_actionset *)apr_pcalloc(engine->mp, sizeof(msre_actionset));
    if (actionset == NULL) {
        return NULL;
    }
                    ...
                    msre_actionset *child, int inherit_by_default)
{
    ...
    if (child == NULL) {
        /* The child actionset does not exist, hence go with the parent one. */
        return merged;
    }
    ...
    /* Other */
    if (child->attacklog != NOT_SET) merged->attacklog = child->attacklog;
...
msre_actionset *msre_actionset_create_default(msre_engine *engine) 
{
    ...
    return msre_actionset_create(engine, "phase:2,log,auditlog,pass", &my_error_msg);
}
...
void msre_actionset_set_defaults(msre_actionset *actionset) 
{
    ...
    /* Other */
    if (actionset->attacklog == NOT_SET) actionset->attacklog = 1;
...
msre_engine *msre_engine_create(apr_pool_t *parent_pool) 
{
    ...
    if (apr_pool_create(&mp, parent_pool) != APR_SUCCESS) {
        return NULL;
    }
    ...
    if (engine->operators == NULL) {
        return NULL;
    }
    
    engine->variables = apr_table_make(mp, 100);
    if (engine->variables == NULL) {
        return NULL;
    }
    ...
    if (engine->reqbody_processors == NULL) {
        return NULL;
    }
...
apr_status_t msre_ruleset_process_phase(msre_ruleset *ruleset, modsec_rec *msr) 
{
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (rule->placeholder == RULE_PH_MARKER) {
            continue;
        }
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (mode == SKIP_RULES) {
            ...
            if ((rule->placeholder == RULE_PH_NONE) 
                    ...
                    || (strcmp(skip_after, rule->actionset->id) != 0)) {
                if(i - 1 >= 0) {
                    last_rule = rules[i - 1];
                } else {
                    last_rule = rules[0];
                ...
                if((last_rule != NULL) && (last_rule->actionset != NULL) 
                        && last_rule->actionset->is_chained && (saw_starter == 1)) {
                    mode = NEXT_RULE;
                    skipped = 1;
                    --i;
                } else {
                    ...
                    if (msr->txcfg->debuglog_level >= 9) {
                        msr_log(msr, 9, "Current rule is id=\"%s\" [chained %d] is trying to find "
                            "the SecMarker=\"%s\" [stater %d]",rule->actionset->id,
                            last_rule->actionset->is_chained,skip_after,saw_starter);
                    }
                }
        ...
        if (((rule->actionset->id !=NULL) && !apr_is_empty_array(msr->removed_rules)) 
                || (apr_is_empty_array(msr->removed_rules_tag)==0)) {
            int j, act;
            int do_process = 1;
            const char *range;
            ...
            for(j = 0; j < msr->removed_rules->nelts; j++) {
                ...
                if(rule->actionset->id !=NULL)  {
                    if (msr->txcfg->debuglog_level >= 9) {
                        msr_log(msr, 9, "Checking removal of rule id=\"%s\" against: %s", 
                            rule->actionset->id, range);
                    }
            ...
            if (do_process == 0) {
                if (msr->txcfg->debuglog_level >= 5) {
                    msr_log(msr, 5, "Not processing %srule id=\"%s\": removed by ctl action",
                        rule->actionset->is_chained ? "chained " : "", rule->actionset->id);
                }
        ...
        if (msr->txcfg->debuglog_level >= 4) {
            ...
            msr_log(msr, 4, "Recipe: Invoking rule %pp;%s%s%s.", rule, (fn ? fn : ""), 
                (id ? id : ""), (rev ? rev : ""));
        ...
        if (rc == RULE_NO_MATCH) {
            if (rule->actionset->is_chained) {
                ...
                /* This rule is not part of a chain so we simply move to the next rule. */
                mode = NEXT_RULE;
                if (msr->txcfg->debuglog_level >= 9) {
            ...
            if (msr->rule_was_intercepted) {
                ...
                if((last_rule != NULL) && (last_rule->actionset != NULL) 
                        && last_rule->actionset->is_chained) {
                    int st = 0;
                    for(st=i;st>=0;st--)  {
                        rule_starter = rules[st];
                        if(rule_starter != NULL && rule_starter->chain_starter != NULL)    {
                            if((msr != NULL) && (msr->intercept_actionset != NULL) 
                                    && (rule_starter->actionset != NULL)) {
            ...
            /* We had a match but the transaction was not intercepted. In that case we proceed with the
             * next rule...
             */
            mode = NEXT_RULE;
            if (msr->txcfg->debuglog_level >= 9) {
            ...
            /* ...unless we need to skip, in which case we determine how many rules/chains we need to
            ...
            if (msr->txcfg->reqintercept_oe == 1)   {
                ...
                if (rule->actionset->is_chained) {
                    /* If the current rule is part of a chain then we need to skip over all the
                     * rules in the chain. 
                     */
                    mode = NEXT_CHAIN;
                    if (msr->txcfg->debuglog_level >= 9) {
...
/**
 * Creates a ruleset that will be handled by the default implementation.
 */
                    ...
                    const apr_array_header_t *phase_arr, int offset)
{
    ...
    for (i = 0; i < phase_arr->nelts; i++) {
        ...
        if ((rule->actionset != NULL) && (!rule->actionset->is_chained || !rule->chain_starter) 
                && (rule->actionset->id != NULL) && (strcmp(rule->actionset->id, id) == 0)) {
...
static int msre_ruleset_phase_rule_remove_with_exception(msre_ruleset *ruleset, 
            rule_exception *re, apr_array_header_t *phase_arr)
{
    msre_rule **rules;
    ...
    for (i = 0; i < phase_arr->nelts; i++) {
        ...
        if (mode == 0) { /* Looking for next rule. */
            ...
            if (rule->placeholder == RULE_PH_NONE) {
                ...
                case RULE_EXCEPTION_REMOVE_MSG :
                    if ((rule->actionset != NULL)&&(rule->actionset->msg != NULL)) {
                        ...
                        int rc = msc_regexec(re->param_data, rule->actionset->msg, 
                ...
                case RULE_EXCEPTION_REMOVE_TAG :
                    if ((rule->actionset != NULL)
                            &&(apr_is_empty_table(rule->actionset->actions) == 0)) {
                        char *my_error_msg = NULL;
                        const apr_array_header_t *tarr = NULL;
                        const apr_table_entry_t *telts = NULL;
                        int act;
                        ...
                        for (act = 0; act < tarr->nelts; act++) {
                            ...
                            if((action != NULL) && 
                               (action->metadata != NULL) && 
                               (strcmp("tag", action->metadata->name) == 0))  {
                                int rc = msc_regexec(re->param_data, action->param, 
            ...
            if (remove_rule) {
                ...
                removed_count++;
                if (rule->actionset->is_chained) {
                    mode = 2; /* Remove rules in this chain. */
                }
            } else {
                if (rule->actionset->is_chained) {
                    mode = 1; /* Keep rules in this chain. */
                }
                rules[j++] = rules[i];
            }
...
char *msre_format_metadata(modsec_rec *msr, msre_actionset *actionset) 
{
    ...
    if ((actionset->rule != NULL) && (actionset->rule->filename != NULL)) {
        fn = apr_psprintf(msr->mp, " [file \"%s\"] [line \"%d\"]", actionset->rule->filename, 
            actionset->rule->line_num);
    }
    if (actionset->id != NULL) {
        id = apr_psprintf(msr->mp, " [id \"%s\"]", log_escape(msr->mp, actionset->id));
    }
    if (actionset->rev != NULL) {
        rev = apr_psprintf(msr->mp, " [rev \"%s\"]", log_escape(msr->mp, actionset->rev));
    }
    ...
    if (actionset->msg != NULL) {
        ...
        msg = apr_psprintf(msr->mp, " [msg \"%s\"]", log_escape_ex(msr->mp, var->value, 
                var->value_len));
    }
    ...
    if (actionset->logdata != NULL) {
        ...
        /* If it is > 512 bytes, then truncate at 512 with ellipsis. NOTE: 512 actual data + 9 
         * bytes of label = 521 */
    ...
    if ((actionset->severity >= 0)&&(actionset->severity <= 7)) {
        severity = apr_psprintf(msr->mp, " [severity \"%s\"]", msre_format_severity(actionset->severity));
    }
    ...
    for (k = 0; k < tarr->nelts; k++) {
        ...
        if (strcmp(telts[k].key, "tag") == 0) {
            ...
            tags = apr_psprintf(msr->mp, "%s [tag \"%s\"]", tags, log_escape(msr->mp, var->value));
        }
    }
        ...
        const char *args, const char *actions)
{
    ...
    if (r_args == NULL) {
        r_args = apr_pstrcat(pool, (rule->op_negated ? "!" : ""), "@", rule->op_name, " ", 
                    rule->op_param, NULL);
    }
    ...
    case RULE_TYPE_NORMAL:
        if (r_actions == NULL) {
            unparsed = apr_psprintf(pool, "SecRule \"%s\" \"%s\"", log_escape(pool, r_targets), 
                log_escape(pool, r_args));
        } else {
            unparsed = apr_psprintf(pool, "SecRule \"%s\" \"%s\" \"%s\"", log_escape(pool, r_targets), 
                log_escape(pool, r_args), log_escape(pool, r_actions));
        }
        break;
        
    case RULE_TYPE_ACTION:
        unparsed = apr_psprintf(pool, "SecAction \"%s\"", log_escape(pool, r_actions));
        break;
    ...
    case RULE_TYPE_LUA:
        ...
        if (r_actions == NULL) {
            ...
            unparsed = apr_psprintf(pool, "SecRuleScript \"%s\" \"%s\"", r_args, 
 ...
 */
msre_rule *msre_rule_create(msre_ruleset *ruleset, int type, const char *fn, int line, 
            const char *targets, const char *args, const char *actions, char **error_msg)
{
    msre_rule *rule;
    char *my_error_msg;
    const char *argsp;
    ...
    *error_msg = NULL;
    ...
    if (*argsp != '@') {
        ...
        while(isspace(*p)) {
            p++; /* skip over the whitespace at the end*/
        }
    ...
    if (rule->op_metadata == NULL) {
        *error_msg = apr_psprintf(ruleset->mp, "Error creating rule: Failed to resolve operator: %s", 
            rule->op_name);
        return NULL;
    }
    ...
    /* 创建转换函数集 */
    rule->tfnset = apr_table_make(ruleset->mp, 10);
    if (rule->tfnset == NULL) {
        return NULL;
    }
    ...
    if (actions != NULL) {
        ...
        /* 转换函数解析 */
        rc = msre_tfns_parse(ruleset->engine, actions, rule->tfnset, &my_error_msg);
        if (rc != 0) {
            *error_msg = apr_psprintf(ruleset->mp, "Error parsing tfns: %s", my_error_msg);
            return NULL;
        }
    }
...
#if defined(WITH_LUA)
msre_rule *msre_rule_lua_create(msre_ruleset *ruleset, const char *fn, int line, 
            const char *script_filename, const char *actions, char **error_msg)
{
    msre_rule *rule;
    char *my_error_msg;
 ...
 */
static void msre_perform_disruptive_actions(modsec_rec *msr, msre_rule *rule, msre_var *var,
                msre_actionset *actionset, apr_pool_t *mptmp, const char *message)
{
    ...
    if (actionset->attacklog != 0) {
        msr->is_relevant++;
    }

    /* We only do stuff when in ONLINE mode. In all other cases we only emit warnings. */
...
attack_log:
    ...
    if (msr->txcfg->attacklog_flag && actionset->attacklog) {
#ifdef DEBUG_FILELOG
        *(const char **)apr_array_push(msr->alerts) = msc_alert_message(msr, actionset, NULL, message);
#endif

#ifdef DEBUG_DBLOG
        msc_record_attacklog(msr, var, actionset);
#endif
    }
}
            ...
            msre_actionset *acting_actionset, apr_pool_t *mptmp)
{
    ...
    if ((var->param != NULL) && (var->name != NULL) && (strchr(var->name,':') == NULL)) {
        full_varname = apr_psprintf(mptmp, "%s%s:%s", (var->is_counting ? "&" : ""), var->name, 
                        var->param);
    ...
    if (msr->txcfg->debuglog_level >= 4) {
        msr_log(msr, 4, "Executing operator \"%s%s\" with param \"%s\" against %s.",
            (rule->op_negated ? "!" : ""), rule->op_name, log_escape(msr->mp, rule->op_param), 
            full_varname);
    }
    ...
    if (((rc == 0) && (rule->op_negated == 0)) || ((rc == 1) && (rule->op_negated == 1))) {
        return RULE_NO_MATCH ; /* No match, do nothing. */
    } else {
        /* Match. */
        if (rc == 0) {
            ...
            my_error_msg = apr_psprintf(msr->mp, "Match of \"%s %s\" against \"%s\" required.",
                log_escape(msr->mp, rule->op_name), op_param, log_escape(msr->mp, full_varname));
        }
        ...
        *(const msre_rule **)apr_array_push(msr->matched_rules) = rule;
        ...
        if ((acting_actionset->severity > 0) 
                && (acting_actionset->severity < msr->highest_severity)) {
        ...
        /* Perform disruptive actions, but only if this rule is not part of a chain. */
        if (rule->actionset->is_chained == 0) {
            msre_perform_disruptive_actions(msr, rule, var, acting_actionset, mptmp, my_error_msg);
        }
...
static int get_var_fetch_key(apr_pool_t *pool, msre_var *var, char **key)
{
    if (var->is_counting) {
        *key = apr_pstrcat(pool, "&", var->name,var->param ? ":" : "", var->param ? var->param : "", NULL);
        return VAR_SIMPLE;
    } else if (var->param == NULL) {
        *key = apr_pstrcat(pool, var->name, NULL);
        return var->metadata->type;
    } else if (var->param != NULL && var->param_data == NULL) {
        *key = apr_pstrcat(pool, var->name, ":", var->param, NULL);
        return VAR_SIMPLE;
    } else if (var->param != NULL && var->param_data != NULL) {
        *key = apr_pstrcat(pool, var->name, ":", var->param, NULL);
        return VAR_LIST;
...
static apr_status_t msre_rule_process_normal(msre_rule *rule, modsec_rec *msr) 
{
    ...
    apr_pool_t *varmptmp = msr->var_fetch_cache_mptmp;
    int k;
    int vartype;
    char *varfetchname;
    apr_table_t *varfetchtab = NULL;
    const apr_array_header_t *vararr = NULL;
    const apr_table_entry_t *varte = NULL;
#endif
...
#ifdef VAR_FETCH_CACHE
        if (targets[i]->metadata->is_cacheable == VAR_CACHE) {
            vartype = get_var_fetch_key(varmptmp, targets[i], &varfetchname);
            if (vartype == -1) {
                msr_log(msr, 1, "Unkown var type.");
                return -1;
            }
            
            varfetchtab = (apr_table_t *)apr_hash_get(msr->var_fetch_cache, varfetchname, 
                strlen(varfetchname));
            
            /* 找到变量缓存 */
            if (varfetchtab != NULL) {
                vararr = apr_table_elts(varfetchtab);
                varte = (apr_table_entry_t *)vararr->elts;
                if (targets[i]->is_negated == 0) {
                    for (k = 0; k < vararr->nelts; k++) {
                        apr_table_addn(tartab, varte[k].key, (void *)varte[k].val);
                    }
                } else {
                    for (k = 0; k < vararr->nelts; k++) {
                        apr_table_unset(tartab, varte[k].key);
                    }
                }
                continue;
            }
            ...
            /* 没有变量缓存 */
            if (vartype == VAR_LIST) {
                varfetchtab = apr_table_make(varmptmp, 24);
            } else {
                varfetchtab = apr_table_make(varmptmp, 1);           
            }
            if (varfetchtab == NULL) {
                return -1;
            } 
            
            apr_hash_set(msr->var_fetch_cache, varfetchname, strlen(varfetchname), (const void *)varfetchtab);
        }
#endif
...
#ifdef VAR_FETCH_CACHE        
        list_count = targets[i]->metadata->generate(msr, targets[i], rule, vartab, varmptmp);
#else
...
#ifdef VAR_FETCH_CACHE 
            msre_var *newvar = (msre_var *)apr_pmemdup(varmptmp, targets[i], sizeof(msre_var));
            newvar->value = apr_psprintf(varmptmp, "%d", list_count);
            newvar->value_len = strlen(newvar->value);
            apr_table_addn(tartab, newvar->name, (void *)newvar);
            if (targets[i]->metadata->is_cacheable == VAR_CACHE) {
                apr_table_addn(varfetchtab, newvar->name, (void *)newvar);
            }
#else
...
#ifdef VAR_FETCH_CACHE
                if (targets[i]->metadata->is_cacheable == VAR_CACHE) {
                    apr_table_addn(varfetchtab, te[j].key, te[j].val);
                }
#endif
            }
        }
    ...
    for (i = 0; i < arr->nelts; i++) {
        ...
        if (msr->txcfg->cache_trans != MODSEC_CACHE_DISABLED) {
            ...
            if (var->is_counting) {
                ...
                if (msr->txcfg->debuglog_level >= 9) {
                    msr_log(msr, 9, "CACHE: Disabled - %s is not yet available in phase %d " 
                        "(requires phase %d or later)", var->name, msr->phase, 
                        var->metadata->availability);
                }
                usecache = 0;
            } else if (var->value_len < msr->txcfg->cache_trans_min) {/* check the cache options */           
                if (msr->txcfg->debuglog_level >= 9) {
                    msr_log(msr, 9, "CACHE: Disabled - %s value length=%u, smaller than minlen=%" 
                        APR_SIZE_T_FMT, var->name, var->value_len, msr->txcfg->cache_trans_min);
                }
                usecache = 0;
            } else if ((msr->txcfg->cache_trans_max != 0) && (var->value_len > msr->txcfg->cache_trans_max)) {
                if (msr->txcfg->debuglog_level >= 9) {
                    msr_log(msr, 9, "CACHE: Disabled - %s value length=%u, larger than maxlen=%" 
                        APR_SIZE_T_FMT, var->name, var->value_len, msr->txcfg->cache_trans_max);
                }
                usecache = 0;
            }
            ...
            if (usecache) {
                if (var->metadata->is_cacheable == VAR_CACHE) {
                    ...
                    #ifdef CACHE_DEBUG
                    msr_log(msr, 9, "CACHE: Fetching cache entry from hash=%pp: %pp=%s", 
                        msr->tcache, var, var->name);
                    #endif
                    ...
                    #endif
                    {
                        ...
                        #ifdef CACHE_DEBUG
                        msr_log(msr, 9, "CACHE: Created a new cache table %pp for %pp", cachetab, 
                            var->value);
                        #endif
                    }
                } else {
        ...
        /* Transform target. */
        {
            ...
            tarr = apr_table_elts(rule->tfnset);
            telts = (const apr_table_entry_t*)tarr->elts;
            /* Build the final list of transformation functions. */
            for (k = 0; k < tarr->nelts; k++) {
                metadata = (msre_tfn_metadata *)telts[k].val;
                if (strcmp(metadata->name, "none") == 0) {
                ...
                apr_table_addn(normtab, metadata->name, (void *)metadata);

                /* Check the cache, saving the 'most complete' as a starting point */
                if (usecache) {
                    tfnspath = apr_psprintf(mptmp, "%s%s%s", (tfnspath ? tfnspath: ""), (tfnspath ? "," : ""), metadata->name);
            ...
            for (; k < tarr->nelts; k++) {
                ...
                metadata = (msre_tfn_metadata *)telts[k].val;
                tfnchanged = metadata->execute(mptmp, (unsigned char *)var->value, var->value_len, &rval, &rval_length);
                if (tfnchanged < 0) {
                    var->value = ori_value;
                    var->value_len = ori_len;
                    return -1;
                ...
                if (usecache) {
                    ...
                    tfnspath = apr_psprintf(msr->mp, "%s%s%s", (tfnspath ? tfnspath : ""),
                        (tfnspath ? "," : ""), metadata->name);
                    tfnskey = apr_psprintf(msr->mp, "%x;%s", tfnsnum, tfnspath);

                    if ((msr->txcfg->cache_trans_maxitems != 0) && (msr->tcache_items >= msr->txcfg->cache_trans_maxitems)) {
                    ...
                    } else if (msr->txcfg->cache_trans_incremental || (tfnsnum == tarr->nelts)) {
                        /* ENH1: Add flag to vars to tell which ones can change across phases 
                         * store the rest in a global cache 
                         */
                         ...
                         */
                        crec->val = (!lastvarval || tfnchanged)
                                     ? apr_pmemdup(msr->mp, var->value, var->value_len) 
                                     : lastvarval;
                        crec->val_len = changed ? ((!lastvarval || tfnchanged) 
                                                    ? var->value_len 
                                                    : lastvarlen) : 0;
                        ...
                        #ifdef CACHE_DEBUG
                        if (changed) {
                            msr_log(msr, 9, "CACHE: Caching %s=\"%s\" (%pp)", tfnskey,
                                            log_escape_nq_ex(mptmp, crec->val, crec->val_len), var);
                        } else {
                            msr_log(msr, 9, "CACHE: Caching %s=<no change> (%pp)", tfnskey, var);
                        }
                        #endif
...
static apr_status_t msre_rule_process_lua(msre_rule *rule, modsec_rec *msr)
{
    ...
    if (my_error_msg != NULL) {
        ...
        /* Perform disruptive actions, but only if this rule is not part of a chain. */
        if (rule->actionset->is_chained == 0) {
            msre_perform_disruptive_actions(msr, rule, NULL, acting_actionset, msr->msc_rule_mptmp, 
                my_error_msg);
        }
    }
