 ...
 *
 */
void msre_engine_reqbody_processor_register(msre_engine *engine, const char *name, void *fn_init, 
        void *fn_process, void *fn_complete)
{
    msre_reqbody_processor_metadata *metadata = 
        (msre_reqbody_processor_metadata *)apr_pcalloc(engine->mp, sizeof(msre_reqbody_processor_metadata));
    if (metadata == NULL) {
        return;
    }
...
static apr_status_t modsecurity_request_body_start_init(modsec_rec *msr, char **error_msg) 
{
    *error_msg = NULL;
    ...
    if(msr->msc_reqbody_storage == MSC_REQBODY_MEMORY) {
        ...
        msr->msc_reqbody_chunks = apr_array_make(msr->msc_reqbody_mp, 32, sizeof(msc_data_chunk *));
...
apr_status_t modsecurity_request_body_start(modsec_rec *msr, char **error_msg) 
{
    *error_msg = NULL;
    ...
    /* Create a separate memory pool that will be used to allocate structures from 
     * (not data, which is allocated via malloc).
     */
    ...
    if (msr->msc_reqbody_processor != NULL) {
        ...
        if (metadata != NULL) {
            if ((metadata->init != NULL) && (metadata->init(msr, &my_error_msg) < 0)) {
                *error_msg = apr_psprintf(msr->mp, "%s parsing error (init): %s", msr->msc_reqbody_processor, my_error_msg);
 ...
 */
static apr_status_t modsecurity_request_body_store_disk(modsec_rec *msr, const char *data, 
...
apr_status_t modsecurity_request_body_to_stream(modsec_rec *msr, const char *buffer, int buflen, char **error_msg) 
{
    ...
    if (msr->stream_input_data== NULL) {
        ...
        *error_msg = apr_psprintf(msr->mp, "Unable to allocate memory to hold request body on "
            "stream. Asked for %" APR_SIZE_T_FMT " bytes.", msr->stream_input_length + 1);
        return -1;
    }
...
static apr_status_t modsecurity_request_body_end_raw(modsec_rec *msr, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    if (msr->msc_reqbody_buffer == NULL) {
        *error_msg = apr_psprintf(msr->mp, "Unable to allocate memory to hold request body. Asked "
                        "for %u bytes.", msr->msc_reqbody_length + 1);
        return -1;
    }
...
apr_status_t modsecurity_request_body_retrieve_start(modsec_rec *msr, char **error_msg) 
{
    *error_msg = NULL;
    ...
    } else if (msr->msc_reqbody_storage == MSC_REQBODY_DISK) {
        msr->msc_reqbody_disk_chunk = apr_pcalloc(msr->msc_reqbody_mp, sizeof(msc_data_chunk));
        if (msr->msc_reqbody_disk_chunk == NULL) {
            *error_msg = apr_psprintf(msr->mp, "Failed to allocate %lu bytes for request body disk chunk.",
                (unsigned long)sizeof(msc_data_chunk));
 ...
 */
apr_status_t modsecurity_request_body_retrieve(modsec_rec *msr, msc_data_chunk **chunk, 
    ...
    *chunk = NULL;
    ...
    if (msr->msc_reqbody_storage == MSC_REQBODY_MEMORY) {
        ...
        *chunk = msr->msc_reqbody_disk_chunk;
        ...
        /* Advance to the current chunk and position on the next byte we need to send. */
        chunks = (msc_data_chunk **)msr->msc_reqbody_chunks->elts;
        ...
        if (nbytes < 0) {
            ...
            if (chunks[msr->msc_reqbody_chunk_position]->length - msr->msc_reqbody_chunk_offset <= (unsigned int)nbytes) {
                /* If what's left in our chunk is less than the limit then send it all back. */
                msr->msc_reqbody_disk_chunk->length = chunks[msr->msc_reqbody_chunk_position]->length - msr->msc_reqbody_chunk_offset;
                msr->msc_reqbody_chunk_position++;
                msr->msc_reqbody_chunk_offset = 0;
            } else {
                /* If we have more data in our chunk, send the maximum bytes we can (nbytes). */
                msr->msc_reqbody_disk_chunk->length = nbytes;
                msr->msc_reqbody_chunk_offset += nbytes;
            }
        }

        /* If we've advanced beyond our last chunk then we have no more data to send.*/
        if (msr->msc_reqbody_chunk_position >= msr->msc_reqbody_chunks->nelts) {
    ...
    if (msr->msc_reqbody_storage == MSC_REQBODY_DISK) {
        ...
        if (i < 0) {
            *error_msg = apr_psprintf(msr->mp, "Input filter: Error reading from temporary file: %s", strerror(errno));
            return -1;
        }
    ...
    /* Should never happen. */
    *error_msg = apr_psprintf(msr->mp, "Internal error, invalid msc_reqbody_storage value: %u", msr->msc_reqbody_storage);
