                ...
                if (i + 2 < input_len) {
                    ...
                    if (VALID_HEX(c1) && VALID_HEX(c2)) {
                        *d++ = x2c(&input[i + 1]);
                        ...
                        i += 3;
                        *changed = 1;
                        if ((*(d - 1) == '%') && VALID_HEX(input[i]) && VALID_HEX(input[i + 1])) {
                            *decode_again = 1;
                        }
                    } else {
    ...
    while(i < input_len) {
        if ((input[i] == '\\')&&(i + 1 < input_len)) {
            ...
            if (c == -1) {
                ...
                } else if (ISODIGIT(input[i + 1])) { /* Octal. */
                        char buf[4];
                        int j = 0;
...
/**
 * hex to bytes for cookie encryt.
 */
int hex2bytes_inplace_3des(unsigned char *data, int len)
{
	unsigned char *d = data;
    int i, count = 0;
