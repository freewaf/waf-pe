...
static void version(apr_pool_t *mp) 
{
    ...
    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL,
            "ModSecurity: APR compiled version=\"%s\"; loaded version=\"%s\"", APR_VERSION_STRING, 
            apr_version_string());

    if (strstr(apr_version_string(), APR_VERSION_STRING) == NULL) {
        ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "ModSecurity: Loaded APR do not match "
            "with compiled!");
    }
    ...
    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL,
            "ModSecurity: PCRE compiled version=\"%s\"; loaded version=\"%s\"", pcre_vrs, 
            pcre_version());

    if (strstr(pcre_version(),pcre_vrs) == NULL)    {
        ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "ModSecurity: Loaded PCRE do not match "
            "with compiled!");
    }

/* Lua version function was removed in current 5.1. Need to check in future versions if it's back */
#if defined(WITH_LUA)
    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL, "ModSecurity: LUA compiled version=\"%s\"", LUA_VERSION);
#endif /* WITH_LUA */

    ap_log_error(APLOG_MARK, APLOG_NOTICE, 0, NULL, "ModSecurity: LIBXML compiled version=\"%s\"",
...
int perform_interception(modsec_rec *msr) 
{
    ...
    if (actionset->intercept_pause) {
        msr_log(msr, (log_level > 3 ? log_level : log_level + 1), "Pausing transaction for %d msec.", 
            actionset->intercept_pause);
    ...
    case ACTION_DENY :
        if (actionset->intercept_status != 0) {
            ...
            message = apr_psprintf(msr->mp, "Access denied with code %d%s.", status, phase_text);
        } else {
            log_level = 1;
            status = HTTP_INTERNAL_SERVER_ERROR;
            message = apr_psprintf(msr->mp, "Access denied with code 500%s (Internal Error: Invalid "
                        "status code requested %d).", phase_text, actionset->intercept_status);
        }
        break;
    ...
    case ACTION_PROXY :
        if (msr->phase < 3) {
            if (ap_find_linked_module("mod_proxy.c") == NULL) {
                ...
                status = OK;
                message = apr_psprintf(msr->mp, "Access denied using proxy to%s %s.", phase_text, 
                            log_escape_nq(msr->mp, actionset->intercept_uri));
            }
        } else {
            log_level = 1;
            status = HTTP_INTERNAL_SERVER_ERROR;
            message = apr_psprintf(msr->mp, "Access denied with code 500%s (Configuration Error: "
                        "Proxy action requested but it does not work in output phases).", phase_text);
        }
        break;

    case ACTION_DROP :
        /* ENH This does not seem to work on Windows. Is there a better way to drop a connection anyway? */
        #ifndef WIN32
        {
            extern module core_module;
            apr_socket_t *csd = ap_get_module_config(msr->r->connection->conn_config, &core_module);

            if (csd) {
                if (apr_socket_close(csd) == APR_SUCCESS) {
                    status = HTTP_FORBIDDEN;
                    message = apr_psprintf(msr->mp, "Access denied with connection close%s.", phase_text);
                } else {
                    log_level = 1;
                    status = HTTP_INTERNAL_SERVER_ERROR;
                    message = apr_psprintf(msr->mp, "Access denied with code 500%s (Error: "
                        "Connection drop requested but failed to close the  socket).", phase_text);
                }
            } else {
                log_level = 1;
                status = HTTP_INTERNAL_SERVER_ERROR;
                message = apr_psprintf(msr->mp, "Access denied with code 500%s (Error: Connection "
                            "drop requested but socket not found.", phase_text);
            }
        }
        #else
        log_level = 1;
        status = HTTP_INTERNAL_SERVER_ERROR;
        message = apr_psprintf(msr->mp, "Access denied with code 500%s (Error: Connection drop not "
                    "implemented on this platform).", phase_text);
        #endif
        break;
    ...
    case ACTION_REDIRECT :
        ...
        message = apr_psprintf(msr->mp, "Access denied with redirection to %s using status %d%s.",
                    log_escape_nq(msr->mp, actionset->intercept_uri), status, phase_text);
        break;
    ...
    default :
        ...
        status = HTTP_INTERNAL_SERVER_ERROR;
        message = apr_psprintf(msr->mp, "Access denied with code 500%s (Internal Error: invalid "
                    "interception action %d).", phase_text, actionset->intercept_action);
        break;
    }
...
#ifdef DEBUG_FILELOG
    if ((log_level > 3) && (actionset->attacklog != 0)) {
     ...
     */
    if ((actionset->attacklog == 0) && (log_level <= 3)) {
        msr->is_relevant--;
    }
...
static modsec_rec *create_tx_context(request_rec *r) 
{
    ...
    msr->dcfg1 = (directory_config *)ap_get_module_config(r->per_dir_config, &security_module);
    ...
    /* Create a transaction context and populate it using the directory config we just
     * got from Apache.
     */
...
#endif
    msr->attacklogs = apr_array_make(msr->mp, 5, sizeof(char *));
...
static apr_status_t change_server_signature(server_rec *s) 
{
    ...
    if (server_version == NULL) {
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, s, "SecServerSignature: Apache "
            "returned null as signature.");
        return -1;
    }
    ...
    if (strlen(server_version) >= strlen(new_server_signature)) {
        ...
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, s, "SecServerSignature: original "
            "signature too short. Please set ServerTokens to Full.");
        return -1;
    }
    ...
    if ((server_version == NULL) || (strcmp(server_version, new_server_signature) != 0)) {
        ap_log_error(APLOG_MARK, APLOG_ERR | APLOG_NOERRNO, 0, s, "SecServerSignature: Failed to "
            "change server signature to \"%s\".", new_server_signature);
        return 0;
    } else {
        ap_log_error(APLOG_MARK, APLOG_DEBUG | APLOG_NOERRNO, 0, s, "SecServerSignature: Changed"
            " server signature to \"%s\".", server_version);
    }
...
static int hook_pre_config(apr_pool_t *mp, apr_pool_t *mp_log, apr_pool_t *mp_temp) 
{
    ...
    if (modsecurity == NULL) {
        ap_log_error(APLOG_MARK, APLOG_STARTUP, 0, NULL, "ModSecurity: Failed to initialise engine.");
        return HTTP_INTERNAL_SERVER_ERROR;
    }

    /* 创建unicode map表 */
    if (msc_unicode_map_init(mp) != APR_SUCCESS) {
        return -1;
    }
    

    /* 创建静态ip黑白名单 */
    if (ip_bwlist_create(mp) != APR_SUCCESS) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, 
            "ModSecurity: Failed to initialise ip_bwlist.");
        return -1;
    }

    /* 创建动态ip黑名单 */
    if (ip_dyn_blist_create(mp) != APR_SUCCESS) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, 
            "ModSecurity: Failed to initialise ip_dyn_blist.");
        return -1;
    }

    /* 创建静态url黑白名单 */
    if (url_bwlist_create(mp) != APR_SUCCESS) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, 
            "ModSecurity: Failed to initialise url_bwlist.");
        return -1;
    }

    /* 创建动态refferrer url 黑名单*/
    if (url_dyn_blist_create(mp) != APR_SUCCESS) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, 
            "ModSecurity: Failed to initialise url_dyn_blist.");
        return -1;
    }   

    /* 创建cookie关键字共享内存 */
    if (create_cookie_key_shm(mp) == -1) {
        ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, "ModSecurity: Failed to create cookie key shm.");
        return -1;    
    }
...
/* 安全模块清除处理函数 */
static int hook_clear_handler(apr_pool_t *mp, apr_pool_t *mp_log, apr_pool_t *mp_temp) 
{
    int ret;
    ...
    /* 清除动态ip黑名单定时器线程 */
    if (ip_dyn_blist_timer_destroy() != OK) {
        ret = DECLINED;
    }

    /* 清除动态url黑名单定时器线程 */
    if (url_dyn_blist_timer_destroy() != OK) {
        ret = DECLINED;
    }
    
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, NULL, "Destroy dynamic black list timer.");
    return ret;
}
...
static int hook_post_config(apr_pool_t *mp, apr_pool_t *mp_log, apr_pool_t *mp_temp, server_rec *s) 
{
    ...
    if (init_flag == NULL) {
        first_time = 1;
        apr_pool_userdata_set((const void *)1, "modsecurity-init-flag", apr_pool_cleanup_null, 
            s->process->pool);
    } else {
        rc = modsecurity_init(modsecurity, mp);
        if (rc == -1) {
            ap_log_error(APLOG_MARK, APLOG_ERR, 0, NULL, "Modsecurity init failed.");
        }
    }
    ...
    if (chroot_dir != NULL) {
        /* ENH Is it safe to simply return with an error, instead of using exit()? */
        if (first_time == 0) {
    ...
    if (first_time) {
        ap_log_error(APLOG_MARK, APLOG_NOTICE | APLOG_NOERRNO, 0, s, "%s configured.", 
...
static int hook_request_early(request_rec *r) 
{
    ...
    /* Initialise transaction context and create the initial configuration. */
    msr = create_tx_context(r);
    if (msr == NULL) {
        return DECLINED;
    }
    
    rv = ip_bwlist_proccess(msr);
    if (rv != OK) {     /* IP黑白名单处理 */
        return rv;  
    } 

    if (!msr->white_list) { 
        rv = url_bwlist_process(msr);
        if (rv != OK) { /* URL黑白名单处理 */
            return rv;
        } 
    }
...
static int hook_request_late(request_rec *r) 
{
    ...
    /* Find the transaction context and make sure we are supposed to proceed. */
    msr = retrieve_tx_context(r);
    if (msr == NULL) {
    ...
    /* Hit ip or url black list, skip... */
    if (msr->txcfg->is_enabled == MODSEC_ENABLED && msr->black_list_flag) {
        return DECLINED;
    }
    
    /* Check if it can skip the rule check */
    if (msr->white_list) {
        if (msr->txcfg->debuglog_level >= 4) {
            msr_log(msr, 4, "Skipping request phase rule check.");
        }
        return DECLINED;
    }
    ...
    msr->dcfg2 = (directory_config *)ap_get_module_config(r->per_dir_config, &security_module);
    ...
    if (msr->txcfg->reqbody_access == 1) {
        ...
        if (msr->request_content_length > msr->txcfg->reqbody_limit) {
            if((msr->txcfg->is_enabled == MODSEC_ENABLED) 
                ...
                msr->inbound_error = 1;
                msr_log(msr, 1, "Request body (Content-Length) is larger than the configured limit"
                    " (%ld). Deny with status (%d)", msr->txcfg->reqbody_limit, HTTP_REQUEST_ENTITY_TOO_LARGE);
                return HTTP_REQUEST_ENTITY_TOO_LARGE;
            } else if ((msr->txcfg->is_enabled == MODSEC_ENABLED) 
                    && (msr->txcfg->if_limit_action == REQUEST_BODY_LIMIT_ACTION_PARTIAL)){
                msr->inbound_error = 1;
                msr_log(msr, 1, "Request body (Content-Length) is larger than the configured limit"
                    " (%ld).", msr->txcfg->reqbody_limit);
                return HTTP_REQUEST_ENTITY_TOO_LARGE;
            } else if ((msr->txcfg->is_enabled == MODSEC_DETECTION_ONLY) 
                    && (msr->txcfg->if_limit_action == REQUEST_BODY_LIMIT_ACTION_PARTIAL)){
                msr_log(msr, 1, "Request body (Content-Length) is larger than the configured limit"
                    " (%ld).", msr->txcfg->reqbody_limit);
                msr->inbound_error = 1;
...
static void hook_error_log(const char *file, int line, int module_index, int level, 
                apr_status_t status, const server_rec *s, const request_rec *r, apr_pool_t *mp, 
                const char *fmt)
#else
...
#endif
{
    ...
    if ((msr == NULL) && ((level & APLOG_LEVELMASK) < APLOG_DEBUG)
    ...
    if (file != NULL) {
        em->file = apr_pstrdup(msr->mp, file);
    }
...
static void sec_guardian_logger(request_rec *r, request_rec *origr, modsec_rec *msr) 
{
    ...
    if ((guardianlog_name == NULL)||(guardianlog_fd == NULL)) {
        return;
    }
     ...
     */
    str2 = apr_psprintf(msr->mp, "%" APR_TIME_T_FMT " %" APR_TIME_T_FMT " \"%s\" %d", duration, 
            apr_time_sec(duration), log_escape(msr->mp, modsec_message), modsec_rating);
    if (str2 == NULL) {
        return;
    }
...
static int hook_log_transaction(request_rec *r) 
{
    ...
    if (!msr->white_list && (msr->txcfg->is_enabled == MODSEC_DETECTION_ONLY || !msr->black_list_flag)) {
        modsecurity_process_phase(msr, PHASE_LOGGING);
    }
    
    /* 访问日志 */
    access_log_process(msr);

    /* 攻击日志，该函数从上一个函数分离 */
    attack_log_process(msr);
...
static void hook_insert_error_filter(request_rec *r) 
{
    ...
    if (msr->of_status != OF_STATUS_COMPLETE) {
        ...
        /* Make a note that the output we will be receiving is a result of error processing. */
        msr->of_is_error = 1;
...
{
    if (modsecurity != NULL) {
        msre_engine_op_register(modsecurity->msre, name, (fn_op_param_init_t)fn_init, 
            (fn_op_execute_t)fn_exec);
    }
}
 ...
 * 获得动态黑白名单的数据
 */
static int dyn_access_list_show(int lst, apr_array_header_t **result)
{
    switch (lst) {
    case IP_BLACK:
        return ip_dyn_blist_get(result);
    case IP_WHITE:
    case URL_BLACK:
        return url_dyn_blist_get(result);
    case URL_WHITE:
    case ALL_LIST:
    default:
        break;
...
static int hook_connection_early(conn_rec *conn)
{
    ...
    if(sb != NULL && (conn_read_state_limit > 0 || conn_write_state_limit > 0)) {
        ...
        if ((conn_read_state_limit > 0) && (ip_count > conn_read_state_limit)) {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "ModSecurity: Access denied with code"
                "400. Too many threads [%ld] of %ld allowed in READ state from %s - Possible DoS "
                "Consumption Attack [Rejected]", ip_count,conn_read_state_limit,conn->remote_ip);
            return OK;
        } else if ((conn_write_state_limit > 0) && (ip_count_w > conn_write_state_limit)) {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, "ModSecurity: Access denied with code"
                " 400. Too many threads [%ld] of %ld allowed in WRITE state from %s - Possible DoS "
                "Consumption Attack [Rejected]", ip_count_w,conn_write_state_limit,conn->remote_ip);
            return OK;
        } else {
            return DECLINED;
        }
 ...
 */
static void modsec_register_variable(const char *name, unsigned int type, unsigned int argc_min, 
                unsigned int argc_max, void *fn_validate, void *fn_generate, 
                unsigned int is_cacheable, unsigned int availability, enum ATTACK_DOMAIN domain) 
{
    if (modsecurity != NULL) {
        msre_engine_variable_register(modsecurity->msre, name, type, argc_min, argc_max, 
            (fn_var_validate_t)fn_validate, (fn_var_generate_t)fn_generate, is_cacheable, availability,
            domain);
    } else {
 ...
 */
static void modsec_register_reqbody_processor(const char *name, void *fn_init, void *fn_process,
                void *fn_complete)
{
    if (modsecurity != NULL) {
...
{
    printf("PE %s Release (%s)\n", MODSEC_VERSION, MODSEC_VERSION_RELEASE);
}
#endif
...
static void register_hooks(apr_pool_t *mp) 
{
    ...
    *(char **)apr_array_push(ap_server_config_defines) = apr_psprintf(mp, "MODSEC_%s.%s", 
        MODSEC_VERSION_MAJOR, MODSEC_VERSION_MINOR);
    ...
    APR_REGISTER_OPTIONAL_FN(dyn_access_list_show);
	APR_REGISTER_OPTIONAL_FN(Print_Version);
#endif
    ...
    /* 模块清除处理钩子 */
    ap_hook_clear_handler(hook_clear_handler, NULL, NULL, APR_HOOK_FIRST);
    
    ap_hook_post_config(hook_post_config, postconfig_beforeme_list, postconfig_afterme_list, 
    ...
    ap_hook_post_read_request(hook_request_early, postread_beforeme_list, postread_afterme_list, 
