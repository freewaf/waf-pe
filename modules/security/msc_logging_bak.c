...
void sec_audit_logger(modsec_rec *msr)
{
    ...
    if (msr->txcfg->auditlog_type != AUDITLOG_CONCURRENT) {
        ...
        if (rc != APR_SUCCESS) {
            msr_log(msr, 1, "Audit log: Failed to lock global mutex: %s", get_apr_error(msr->mp, rc));
        }
    }
    ...
    text = apr_psprintf(msr->mp, "[%s] %s %s %u %s %u", current_logtime(msr->mp), msr->txid, 
            msr->remote_addr, msr->remote_port, msr->local_addr, msr->local_port);
    sec_auditlog_write(msr, text, strlen(text));
            ...
            for(;;) {
                ...
                for(i = 0; i < tarr->nelts; i++) {
                    ...
                    if (last_offset == 0) { /* The first time we're here. */
                        ...
                        if ((arg->value_origin_offset > offset) &&(arg->value_origin_offset < last_offset)) {
                            offset = arg->value_origin_offset;
                            nextarg = arg;
                        }
                    }
    ...
    if (strchr(msr->txcfg->auditlog_parts, AUDITLOG_PART_A_RESPONSE_HEADERS) != NULL) {
        ...
        if (msr->response_headers_sent) {
            if (msr->status_line != NULL) {
                text = apr_psprintf(msr->mp, "%s %s\n", msr->response_protocol, msr->status_line);
            } else {
                text = apr_psprintf(msr->mp, "%s %u\n", msr->response_protocol, msr->response_status);
            }
            sec_auditlog_write(msr, text, strlen(text));
            ...
            if (parts[cfiles]->type == MULTIPART_FILE) {
                if(parts[cfiles]->filename != NULL) {
                    text = apr_psprintf(msr->mp, "%d,%u,\"%s\",\"%s\"\n", cfiles+1, 
                        parts[cfiles]->tmp_file_size, log_escape(msr->mp, parts[cfiles]->filename), 
                        log_escape(msr->mp, parts[cfiles]->content_type ? parts[cfiles]->content_type : "<Unknown ContentType>"));
                    sec_auditlog_write(msr, text, strlen(text));
