    ...
    long int input_len, char **rval, long int *rval_len)
{
    ...
    length = urldecode_uni_nonstrict_inplace_ex(input, input_len, &changed, &decode_again);
    *rval = (char *)input;
    *rval_len = length;
...
/* urlDecodes */
static int msre_fn_urldecodes_execute(apr_pool_t *mptmp, unsigned char *input,
    long int input_len, char **rval, long int *rval_len)
{
    long int length;
    ...
    decode_again = 0;
    length = urldecode_uni_nonstrict_inplace_ex(input, input_len, &changed1, &decode_again);
    if (decode_again) {
        length = urldecode_uni_nonstrict_inplace_ex(input, length, &changed2, &decode_again);
    }
    *rval = (char *)input;
    *rval_len = length;
...
* \retval 1 On Success
*/
static int msre_fn_base64DecodeExt_execute(apr_pool_t *mptmp, unsigned char *input, 
            long int input_len, char **rval, long int *rval_len)
{
    *rval_len = input_len;
    *rval = apr_palloc(mptmp, *rval_len);
...
{
    msre_tfn_metadata *metadata = (msre_tfn_metadata *)apr_pcalloc(engine->mp, sizeof(msre_tfn_metadata));
    if (metadata == NULL) {
        return;
    }
