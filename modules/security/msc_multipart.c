...
static int multipart_process_part_header(modsec_rec *msr, char **error_msg)
{
    ...
    *error_msg = NULL;
    ...
    if (((msr->mpd->buf[0] == '\r') && (msr->mpd->buf[1] == '\n') &&(msr->mpd->buf[2] == '\0') )
            || ((msr->mpd->buf[0] == '\n') && (msr->mpd->buf[1] == '\0'))) { /* Empty line. */
        ...
        if (isspace(msr->mpd->buf[0])) {
            ...
            if (msr->txcfg->debuglog_level >= 9) {
                msr_log(msr, 9, "Multipart: Added part header \"%s\" \"%s\"",
                    log_escape(msr->mp, header_name), log_escape(msr->mp, header_value));
            }
        }
    }
...
static int multipart_process_part_data(modsec_rec *msr, char **error_msg) 
{
    ...
    *error_msg = NULL;
    ...
    if (((MULTIPART_BUF_SIZE - msr->mpd->bufleft) >= 1) && (*(p - 1) == '\n')) {
        if (((MULTIPART_BUF_SIZE - msr->mpd->bufleft) >= 2) && (*(p - 2) == '\r')) {
            /* Two bytes. */
            bytes_reserved = 2;
            localreserve[0] = *(p - 2);
            localreserve[1] = *(p - 1);
    ...
    if (msr->mpd->mpp->type == MULTIPART_FILE) {
        ...
        if (extract && (msr->mpd->nfiles >= msr->txcfg->upload_file_limit)) {
            if (msr->mpd->flag_file_limit_exceeded == 0) {
                *error_msg = apr_psprintf(msr->mp, "Multipart: Upload file limit exceeded "
                            "SecUploadFileLimit %d.", msr->txcfg->upload_file_limit);
        ...
        if (extract) {
            ...
            if (msr->mpd->mpp->tmp_file_fd == 0) {
                ...
                msr->mpd->mpp->tmp_file_fd = msc_mkstemp_ex(msr->mpd->mpp->tmp_file_name, 
                    msr->txcfg->upload_filemode);
                ...
                if (msr->txcfg->debuglog_level >= 4) {
                    msr_log(msr, 4, "Multipart: Created temporary file %d (mode %04o): %s",
                        msr->mpd->nfiles, (unsigned int)msr->txcfg->upload_filemode,
                        log_escape_nq(msr->mp, msr->mpd->mpp->tmp_file_name));
                }
            }
            ...
            if (msr->mpd->reserve[0] != 0) {
                if (write(msr->mpd->mpp->tmp_file_fd, &msr->mpd->reserve[1], 
                            msr->mpd->reserve[0]) != msr->mpd->reserve[0]) {
        ...
        if (msr->mpd->reserve[0] != 0) {
            value_part->data = apr_palloc(msr->mp, (MULTIPART_BUF_SIZE - msr->mpd->bufleft) + 
                                msr->mpd->reserve[0]);

            memcpy(value_part->data, &(msr->mpd->reserve[1]), msr->mpd->reserve[0]);
            memcpy(value_part->data + msr->mpd->reserve[0], msr->mpd->buf, (MULTIPART_BUF_SIZE - 
                        msr->mpd->bufleft));
    ...
    /* store the reserved bytes to the multipart context so that they don't get lost */
    if (bytes_reserved) {
...
static int multipart_process_boundary(modsec_rec *msr, int last_part, char **error_log) 
{
    ...
    if (msr->mpd->mpp != NULL) {
        /* close the temp file */
        if ((msr->mpd->mpp->type == MULTIPART_FILE) && (msr->mpd->mpp->tmp_file_name != NULL)
        ...
        if (msr->mpd->mpp->name) {
            ...
            *(multipart_part **)apr_array_push(msr->mpd->parts) = msr->mpd->mpp;
            ...
            msr_log(msr, 3, "Multipart: Skipping invalid part %pp (part name missing): "
                "(offset %u, length %u)", msr->mpd->mpp, msr->mpd->mpp->offset, msr->mpd->mpp->length);
        }
    ...
    *error_msg = NULL;
    ...
    if (msr->mpd->boundary != NULL) {
        ...
        if (b != (msr->mpd->boundary + 8)) {
            /* Check all characters between the end of the boundary and the = character. */
        ...
        if (msr->txcfg->debuglog_level >= 9) {
            msr_log(msr, 9, "Multipart: Boundary%s: %s", (msr->mpd->flag_boundary_quoted ? " (quoted)" : ""),
...
int multipart_complete(modsec_rec *msr, char **error_msg) 
{
    ...
    if ((msr->mpd->seen_data != 0) && (msr->mpd->is_complete == 0)) {
        if (msr->mpd->boundary_count > 0) {
            /* Check if we have the final boundary (that we haven't processed yet) in the buffer.*/
 ...
 *
 */
int multipart_process_chunk(modsec_rec *msr, const char *buf, unsigned int size, char **error_msg)
{
    char *inptr = (char *)buf;
    ...
    *error_msg = NULL;
    ...
    if (msr->mpd->bufleft == 0) {
        msr->mpd->flag_error = 1;
        *error_msg = apr_psprintf(msr->mp, "Multipart: Internal error in process_chunk: no space "
                        "left in the buffer");
        return -1;
    }
    ...
    while(inleft > 0) {
        ...
        /* until we either reach the end of the line or the end of our internal buffer */
        /* 是够到达一行的末尾，是否到达内部buf的末尾 */
        if ((c == '\n') || (msr->mpd->bufleft == 0) || (process_buffer)) {
            ...
            *(msr->mpd->bufptr) = 0;
            ...
            /* 判断是否为boundary标志 --|----------------xxxxx 中的--*/
            if (msr->mpd->buf_contains_line && (strlen(msr->mpd->buf) > 3) 
                    && (*(msr->mpd->buf) == '-') && (*(msr->mpd->buf + 1) == '-')) {
                /* Does it match our boundary? */
                /* 匹配boundary标志 --|----------------xxxxx 的 ----------------xxxxx*/
                if ((strlen(msr->mpd->buf) >= strlen(msr->mpd->boundary) + 2)
                    ...
                    if (((*boundary_end == '\r') && (*(boundary_end + 1) == '\n') 
                            && (*(boundary_end + 2) == '\0'))
                            || ((*boundary_end == '\n') && (*(boundary_end + 1) == '\0'))) {
                        if (*boundary_end == '\n') {
                            msr->mpd->flag_lf_line = 1;
                        ...
                        /* error */
                        msr->mpd->flag_error = 1;
                        *error_msg = apr_psprintf(msr->mp, "Multipart: Invalid boundary: %s",
                            log_escape_nq(msr->mp, msr->mpd->buf));
                        return -1;
                    }
                    ...
                    if (   (msr->mpd->flag_boundary_quoted)
                        ...
                        && (*(msr->mpd->buf + 2) == '"')
                        && (strncmp(msr->mpd->buf + 3, msr->mpd->boundary, 
                                strlen(msr->mpd->boundary)) == 0)) {
                    ...
                    if ((p != msr->mpd->buf + 2) && (strncmp(p, msr->mpd->boundary, 
                            strlen(msr->mpd->boundary)) == 0)) {
                ...
                /* Look into the buffer to see if there's anything there that resembles a boundary.*/
                if (msr->mpd->buf_contains_line) {
                ...
                if (msr->mpd->mpp == NULL) {
                    ...
                    if (msr->mpd->mpp_state == 0) {
                        if ((msr->mpd->bufleft == 0) || (process_buffer)) {
                            /* part header lines must be shorter than MULTIPART_BUF_SIZE bytes */
                            msr->mpd->flag_error = 1;
                            *error_msg = apr_psprintf(msr->mp, "Multipart: Part header line over "
                                "%d bytes long", MULTIPART_BUF_SIZE);
                            return -1;
                        }
            ...
            /* Update the offset of the data we are about to process. This is to allow us to know 
             * the offsets of individual files and variables.
             */
            msr->mpd->buf_offset += (MULTIPART_BUF_SIZE - msr->mpd->bufleft);

            /* reset the pointer to the beginning of the buffer and continue to accept input data */
            msr->mpd->bufptr = msr->mpd->buf;
            msr->mpd->bufleft = MULTIPART_BUF_SIZE;
...
apr_status_t multipart_cleanup(modsec_rec *msr)
{
    ...
    if (msr->upload_remove_files == 0) {
        if (msr->txcfg->upload_dir == NULL) {
            msr_log(msr, 1, "Input filter: SecUploadDir is undefined, unable to store multipart files.");
        } else {
            keep_files = 1;
        }
    }
