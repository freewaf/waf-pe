                           ...
                           const char *fmt, va_list args)
{
    ...
    gmtime_r(&time_current, &systime);
    if ((level & APLOG_STARTUP) != APLOG_STARTUP) {
        len += apr_snprintf(errstr + len, MAX_STRING_LEN - len, "[%02d:%02d:%02d.%06d][%5lu][%5lu] "
            , (systime.tm_hour + 8)
            , systime.tm_min
            , systime.tm_sec
            , (apr_int32_t)nowtimeval.tv_usec
            , (unsigned long)getpid(), 
            (unsigned long)syscall(SYS_gettid));
        len += apr_snprintf(errstr + len, MAX_STRING_LEN - len,
                            "[%6s] ", priorities[level_and_mask].t_name);
    }
