...
#if defined(APR_TCP_NODELAY) && !defined(MPE) && !defined(TPF)
...
int ap_signal_server(int *exit_status, apr_pool_t *pconf)
{
    ...
    *exit_status = 0;
    ...
    if (rv != APR_SUCCESS) {
        ...
        status = "pe (no pid file) not running";
    }
    else {
        if (kill(otherpid, 0) == 0) {
            running = 1;
            status = apr_psprintf(pconf,
                                  "pe (pid %" APR_PID_T_FMT ") already "
                                  "running", otherpid);
        }
        else {
            status = apr_psprintf(pconf,
                                  "pe (pid %" APR_PID_T_FMT "?) not running",
                                  otherpid);
        }
    }
    ...
    if (!strcmp(dash_k_arg, "restart")) {
        if (!running) {
            printf("pe not running, trying to start\n");
        }
        else {
...
#else
        printf("pe MPM \"" MPM_NAME "\" does not support graceful-stop\n");
#endif
        return 1;
    }
