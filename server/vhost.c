                                 ...
                                 ipaddr_chain *ic)
{
    ...
    if (ic->names || ic->server == NULL) {
        ...
        if (sar->host_port != ic->sar->host_port) {
            /* if have " NameVirtualHost * " but no <VirtualHost _default_:*> match for it */
        ...
        for (sar = s->addrs; sar; sar = sar->next) {
            ...
            if ((sar->host_addr->family == AF_INET &&
                ...
                if (!ic) {
                    ...
                    #if 1
                    /* have more than a same default host */
                    ap_log_error(APLOG_MARK, APLOG_WARNING,
                                 0, main_s, "_default_ VirtualHost "
                                 "overlap on port %u, the first has "
                                 "precedence", sar->host_port);
                    #endif
                    /* 把之前基于IP的默认虚拟主机自动转换为基于域名的默认虚拟主机 */
                    nc = new_name_chain(p, ic->server, ic->sar);
                    nc->next = ic->names;
                    ic->names = nc;
                    /* 把当前的虚拟主机加入到基于域名的虚拟主机链表 */
...
static int check_hostalias(request_rec *r)
{
    /*
...
static int check_serverpath(request_rec *r)
{
    server_rec *s;
    server_rec *last_s;
    name_chain *src;
...
AP_DECLARE(void) ap_update_vhost_from_headers(request_rec *r)
{
    ...
    if (r->hostname || (r->hostname = apr_table_get(r->headers_in, "Host"))) {
        ...
        if (r->status != HTTP_OK) {
            if (r->connection->vhost_lookup_data) {
                /* 对于Host错误的情况，使用默认虚拟主机 */
                port = r->connection->local_addr->port;
                trav = find_default_server(port);
                if (trav) {
                    /* s is the first matching server, we're done */
                    r->server = trav->server;
                }
                else {
                    /*没有默认虚拟主机的时候，使用主服务器 */
                    r->server = ap_main_server;
                }

                r->connection->vhost_lookup_data = NULL;
            }
            return;
        }
    }
    ...
    if (r->connection->vhost_lookup_data) {
        if (r->hostname) {
            rv = check_hostalias(r);
        }
        else {
            rv = check_serverpath(r);
        }

        if (rv != TRUE) {
            /* 如果找不到对应的虚拟主机，使用默认虚拟主机 */
            port = r->connection->local_addr->port;
            trav = find_default_server(port);
            if (trav) {
                /* s is the first matching server, we're done */
