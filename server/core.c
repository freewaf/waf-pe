...
AP_DECLARE_DATA ap_filter_rec_t *ap_core_offline_input_filter_handle;
 ...
 * ServerName www.example.com:80s
...
static int core_create_offline_req(request_rec *r, request_rec *pr)
{
    return core_create_req(pr);
}
                                  ...
                                  apr_bucket_alloc_t *alloc)
{
    ...
    ap_log_error(APLOG_MARK, APLOG_DEBUG, 0, server
                 , "create a connection: remote_ip %s local_ip %s local_port %d, remote_port %d"
                 , c->remote_ip
                 , c->local_ip
                 , c->local_addr->port
                 , c->remote_addr->port);
...
static int core_offline_pre_connection(conn_rec *c, void *csd)
{
    core_net_rec *net = apr_palloc(c->pool, sizeof(*net));
    apr_status_t rv;
    ...
    ap_set_module_config(net->c->conn_config, &core_module, csd);
    ap_add_input_filter_handle(ap_core_offline_input_filter_handle, net, NULL, net->c);
    ap_add_output_filter_handle(ap_core_output_filter_handle, net, NULL, net->c);
    return DONE;
}
...
static void register_hooks(apr_pool_t *p)
{
    ...
    ap_hook_pre_connection(core_pre_connection, NULL, NULL,
                           APR_HOOK_REALLY_LAST);
    APR_OPTIONAL_HOOK(offline, pre_connection, core_offline_pre_connection, NULL, NULL,
                      APR_HOOK_MIDDLE);
    ...
    APR_OPTIONAL_HOOK(offline, create_req, core_create_offline_req, NULL, NULL,
                      APR_HOOK_MIDDLE);
    ...
    ap_core_offline_input_filter_handle =
    ap_register_input_filter("CORE_IN_OFFLINE", ap_core_offline_input_filter,
                             NULL, AP_FTYPE_NETWORK);
