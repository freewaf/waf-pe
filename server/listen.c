...
#ifdef APR_HAS_LINUX_TPROXY
    /* tproxy:任何模式下都需要设置APR_IP_TRANSPARENT选项，否则子进程会挂掉 */
    stat = apr_socket_opt_set(s, APR_IP_TRANSPARENT, one);
    if (stat != APR_SUCCESS && stat != APR_ENOTIMPL) {
        ap_log_perror(APLOG_MARK, APLOG_CRIT, stat, p,
                      "make_sock: for address %pI, apr_socket_opt_set: (IP_TRANSPARENT)",
                      server->bind_addr);
        apr_socket_close(s);
        return stat;
    }
...
AP_DECLARE_NONSTD(const char *) ap_add_listener(server_rec *s, char *host,
                                                apr_port_t port, const char* protocol)
{
    char *addr, *proto;
    ...
    if (host && !strcmp(host, "*")) {
        ...
        addr = apr_pstrdup(s->process->pconf, host);
    }
    ...
    if (protocol == NULL) {
        ...
        proto = apr_pstrdup(s->process->pconf, protocol);
        ap_str_tolower(proto);
    }

    return alloc_listener(s->process, addr, port, proto);
}
