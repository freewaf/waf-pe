...
static void core_walk_through_brigade(conn_rec * c, apr_bucket_brigade * b, const char * extra)
{
    apr_bucket * e;
    for (e = APR_BRIGADE_FIRST(b);
        e != APR_BRIGADE_SENTINEL(b);
        e = APR_BUCKET_NEXT(e)) {
        ap_log_cerror(APLOG_MARK, APLOG_DEBUG, 0, c, "[%s] core_walk_through_brigade len %d, type SOCKET %d, OFFLINE_SOCKET %d, HEAP %d, METADATA %d, FLUSH %d, EOS %d, FILE %d, PIPE %d, TRANSIENT %d, IMMORTAL %d, MMAP %d, POOL %d"
            , extra
            , (apr_uint32_t)e->length
            , APR_BUCKET_IS_SOCKET(e)
            , APR_BUCKET_IS_OFFLINE_SOCKET(e)
            , APR_BUCKET_IS_HEAP(e)
            , APR_BUCKET_IS_METADATA(e)
            , APR_BUCKET_IS_FLUSH(e)
            , APR_BUCKET_IS_EOS(e)
            , APR_BUCKET_IS_FILE(e)
            , APR_BUCKET_IS_PIPE(e)
            , APR_BUCKET_IS_TRANSIENT(e)
            , APR_BUCKET_IS_IMMORTAL(e)
            , APR_BUCKET_IS_MMAP(e)
            , APR_BUCKET_IS_POOL(e)
            );
    }
}

int ap_core_offline_input_filter(ap_filter_t *f, apr_bucket_brigade *b,
                         ap_input_mode_t mode, apr_read_type_e block,
                         apr_off_t readbytes)
{
    apr_bucket *e;
    ...
    if (!ctx)
    {
        ...
        ctx->tmpbb = apr_brigade_create(ctx->b->p, ctx->b->bucket_alloc);
        /* seed the brigade with the client socket. */
        e = apr_bucket_offline_socket_create(net->client_socket, f->c->bucket_alloc);
        APR_BRIGADE_INSERT_TAIL(ctx->b, e);
        net->in_ctx = ctx;
    }
    else if (APR_BRIGADE_EMPTY(ctx->b)) {
